...
public class AbilityHandler extends Handler {
	...
	public static void handleData(String serverData){
		...
		if(serverData.startsWith("<statuseffect_hit>")){
			...
			if(TARGET.getCreatureType() == CreatureType.Player && TARGET.getDBId() == BlueSaga.playerCharacter.getDBId()){
		...
		if(serverData.startsWith("<use_ability>")){
			...
			if(CASTER != null){
				...
				if(CASTER.getCreatureType() == CreatureType.Player && CASTER.getDBId() == BlueSaga.playerCharacter.getDBId()){
		...
		if(serverData.startsWith("<projectile>")){
			...
			Projectile newProjectile = new Projectile(projectileId, startX, startY, startZ, goalX, goalY, delay, speed, effectId);
			...
			for(String tileInfo: tileData){
				...
				for(String seInfo: seToRemove){
					...
					if(ScreenHandler.SCREEN_TILES.get(tileX+","+tileY+","+tileZ) != null){
						ScreenHandler.SCREEN_TILES.get(tileX+","+tileY+","+tileZ).removeStatusEffect(seId);
					}
				}
			}
		}
