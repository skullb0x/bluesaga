...
public class ConnectHandler extends Handler {
	...
	public static void handleData(String serverData){
		if(serverData.startsWith("<connect>")){
			...
			if(checkversion != ClientSettings.VERSION_NR){
				ScreenHandler.LoadingStatus = "         "+LanguageUtils.getString("ui.status.new_client_available")+"\n"+LanguageUtils.getString("ui.status.visit_site");
			...
			if(!ClientSettings.DEV_MODE){
				...
				for(String emitter: emitterInfo){
					...
					BlueSaga.gameDB.updateDB("INSERT INTO \"Emitter\" VALUES("+emitterData[0]+",'"+emitterData[1]+"',"+emitterData[2]+","+emitterData[3]+","+emitterData[4]+",'"+emitterData[5]+"','"+emitterData[6]+"',"+emitterData[7]+","+emitterData[8]+","+emitterData[9]+","+emitterData[10]+")");
				}
				...
				for(String particle: particleInfo){
					...
					String sql = "INSERT INTO \"Particle\" VALUES("+particleData[0]+",'"+particleData[1]+"','"+particleData[2]+"','"+particleData[3]+"',"+particleData[4]+","+particleData[5]+","+particleData[6]+","+particleData[7]+","+particleData[8]+",'"+particleData[9]+"',"+particleData[10]+","+particleData[11]+",'"+particleData[12]+"','"+particleData[13]+"',"+particleData[14]+","+particleData[15]+")";
					BlueSaga.gameDB.updateDB(sql);
				}
				
				BlueSaga.gameDB.updateDB("CREATE TABLE IF NOT EXISTS projectile (Id INTEGER PRIMARY KEY NOT NULL, GfxName CHAR(20), EmitterId INTEGER DEFAULT 0, HitColor CHAR(20), Sfx CHAR(20))");
				...
				for(String projectile: projectileInfo){
					...
					String sql = "INSERT INTO \"projectile\" VALUES("+projectileData[0]+",'"+projectileData[1]+"',"+projectileData[2]+",'"+projectileData[3]+"','"+projectileData[4]+"')";
					BlueSaga.gameDB.updateDB(sql);
				}
	...
	public static void loadDone() {
		...
		for(Iterator<Npc> iter = BlueSaga.WORLD_MAP.getOtherPlayers().values().iterator();iter.hasNext();){  
