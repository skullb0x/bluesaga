...
public class MonsterHandler extends Handler {
	...
	public static void handleData(String serverData){
		...
		*/
			...
			ScreenHandler.SCREEN_OBJECTS_WITH_ID.get("Monster"+m.getDBId()).getCreature().setRemoved(true);
			if(ScreenHandler.SCREEN_TILES.get(m.getX()+","+m.getY()+","+m.getZ()) != null){
				ScreenHandler.SCREEN_TILES.get(m.getX()+","+m.getY()+","+m.getZ()).setOccupant(CreatureType.None,0);
			}
			MapHandler.updateScreenObjects();
			m.clearDamageLabels();
			m.setDead(false);
