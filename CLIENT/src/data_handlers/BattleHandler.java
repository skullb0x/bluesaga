...
public class BattleHandler extends Handler {
  ...
  public static void handleData(String serverData) {
    ...
    if (serverData.startsWith("<creature_hit>")) {
      ...
      if (damageType.equals("Healing")) {
        ...
        if (TARGET.getCreatureType() == CreatureType.Player
            && BlueSaga.playerCharacter.getDBId() == TARGET.getDBId()) {
    ...
    if (serverData.startsWith("<settarget>")) {
      ...
      // REMOVE OLD TARGET
      if (ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(oldTargetType.toString() + oldTargetId)
          != null) {
        ScreenHandler.SCREEN_OBJECTS_WITH_ID
            .get(oldTargetType.toString() + oldTargetId)
            .getCreature()
            .setAttacked(false);
      }

      // SET NEW TARGET
      if (ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(newTargetType.toString() + newTargetId)
          != null) {
        ScreenHandler.SCREEN_OBJECTS_WITH_ID
            .get(newTargetType.toString() + newTargetId)
            .getCreature()
            .setAttacked(true);
      }
  ...
  public static void showDeath(String victimInfo) {
    ...
    if (VICTIM != null) {
      if (VICTIM.getCreatureType() == CreatureType.Player
          && VICTIM.getDBId() == BlueSaga.playerCharacter.getDBId()) {
        // YOU DIE
        Gui.closeAllWindows();
