...
public class CreatureHandler extends Handler {
	...
	public static void handleData(String serverData){
		...
		if(serverData.startsWith("<creaturepos>")){
			...
			if(removedCreature != null){
				...
				*/
				...
				Tile tile = ScreenHandler.SCREEN_TILES.get(removedCreature.getX()+","+removedCreature.getY()+","+removedCreature.getZ());
		...
		if(serverData.startsWith("<other_unequip>")){
			...
			ScreenHandler.myEmitterManager.SpawnEmitter(creatureEquip.getPixelX()+25, -creatureEquip.getPixelY(), "Equip");
			Sfx.play("gui/equip");
		}
