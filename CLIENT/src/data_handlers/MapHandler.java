...
public class MapHandler extends Handler {
  ...
  public static void handleData(String serverData) {
    if (serverData.startsWith("<fade>")) {
      ...
      ScreenHandler.SCREEN_OBJECTS_WITH_ID.put(
          "Player" + BlueSaga.playerCharacter.getDBId(), newObject);
  ...
  public static Creature addCreatureToScreen(String creatureData) {
    ...
    if (ScreenHandler.SCREEN_OBJECTS_WITH_ID.containsKey(cType.toString() + cDbId)) {
      ScreenHandler.SCREEN_OBJECTS_WITH_ID
          .get(cType.toString() + cDbId)
          .getCreature()
          .setRotationNow(cRotation);
  ...
  public static void updateCreatureOnScreen(String creatureData) {
    ...
    if (ScreenHandler.SCREEN_OBJECTS_WITH_ID.containsKey(creatureType.toString() + cDbId)) {
      Creature newCreature =
          ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(creatureType.toString() + cDbId).getCreature();
  ...
  public static void updateScreenObjects() {
    ...
    for (Iterator<Entry<String, ScreenObject>> iter =
            ScreenHandler.SCREEN_OBJECTS_WITH_ID.entrySet().iterator();
        iter.hasNext();
        ) {
      Entry<String, ScreenObject> entry = iter.next();

      if (isVisibleForPlayer(
          BlueSaga.playerCharacter,
          entry.getValue().getX(),
          entry.getValue().getY(),
          entry.getValue().getZ())) {
                ...
                ScreenHandler.SCREEN_TILES
                    .get(
                        entry.getValue().getX()
                            ...
                            + entry.getValue().getZ())
                    .setOccupant(
                        entry.getValue().getCreature().getCreatureType(),
                        entry.getValue().getCreature().getDBId());
              }
            }
          }
        }
