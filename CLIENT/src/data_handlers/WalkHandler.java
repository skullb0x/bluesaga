...
public class WalkHandler extends Handler {
	...
	public static boolean canWalk(int gotoX, int gotoY){
		...
		if(performWalk && BlueSaga.playerCharacter.getTotalStat("SPEED") > 0){
			...
			Tile GotoTile = ScreenHandler.SCREEN_TILES.get(gotoX+","+gotoY+","+BlueSaga.playerCharacter.getZ());
			if(GotoTile != null){
				
				// DO NOT LET PLAYER GO OUT FROM INN BEFORE TALKING TO INN-KEEPER
				if(Gui.TutorialDialog.isVisible() && Gui.TutorialDialog.getStep() == 1 && !GotoTile.getName().equals("1") && BlueSaga.playerCharacter.getZ() == 1){
					performWalk = false;
				}
				...
				if(GotoTile.getType().contains("shallow")){
					...
					if(!performWalk && !Gui.hasMessage(LanguageUtils.getString("messages.walking.need_boat"))){
						Gui.addMessage("#messages.walking.need_boat", BlueSagaColors.RED);
					}
				}else if(GotoTile.getType().contains("water") && !GotoTile.getType().contains("watercave")){
					performWalk = false;
					if(BlueSaga.playerCharacter.getShip() != null){
					...
					if(!performWalk && !Gui.hasMessage(LanguageUtils.getString("messages.walking.need_better_boat"))){
				...
				// CHECK IF MOVEABLE
				ScreenObject GotoObject = ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(gotoX+","+gotoY+","+BlueSaga.playerCharacter.getZ());
				if(GotoObject != null){
					if(GotoObject.getType().equals("Object")){
						if(GotoObject.getObject().getName().contains("moveable")){
							...
							Tile NextTile = ScreenHandler.SCREEN_TILES.get(nextX+","+nextY+","+BlueSaga.playerCharacter.getZ());
							if(NextTile != null){
								if(!NextTile.isPassable()){
									performWalk = false;
								}else if(ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(nextX+","+nextY+","+BlueSaga.playerCharacter.getZ()) != null){
									performWalk = false;
								}
							}
						}
		...
		if(performWalk){
			...
			// MOVE SCREEN
			BlueSaga.playerCharacter.MyWalkHandler.walkTo(gotoX, gotoY, BlueSaga.playerCharacter.getZ(), BlueSaga.playerCharacter.getTotalStat("SPEED"));
	...
	public static void updatePlayerWalk(PlayerCharacter playerCharacter){
		if(!playerCharacter.MyWalkHandler.isWalking()){
			...
			if(playerCharacter.getGoToTarget()){
				...
				walkPath.clear();
				ScreenObject targetObject = ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(playerCharacter.getAttackTargetType().toString()+playerCharacter.getAttackTargetId());
				...
				if(targetObject != null){
					...
					if(Math.round(Math.sqrt(Math.pow(tileX - playerCharacter.getX(),2) + Math.pow(tileY - playerCharacter.getY(),2))) > playerCharacter.getAttackRange()){
				...
				if(OPEN_CONTAINER){
					...
					if(OPEN_CONTAINER_ID.contains("container")){
						...
						if(OPEN_CONTAINER_ID.contains("_open")){
							...
							BlueSaga.client.sendMessage("gathering",gatherInfo[1]+","+gatherInfo[2]+","+gatherInfo[3]);
						}
	...
	public static void findPath(int tileX, int tileY, int tileZ, String walkType){
		...
		lastPath = new Path();
		lastPath = pathfinder.findPath(pathMover, BlueSaga.playerCharacter.getX() - PathMap.getPathMapStartX(), BlueSaga.playerCharacter.getY() - PathMap.getPathMapStartY(), tileX - PathMap.getPathMapStartX(), tileY - PathMap.getPathMapStartY());
