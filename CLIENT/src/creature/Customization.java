...
public class Customization {
	...
	public void draw(int x, int y, boolean flipped, float aniRotation, float scaleX, float scaleY){
		...
		if(getSkinFeatureId() > 0){
			Image skinGfx = ImageResource.getSprite("items/item"+getSkinFeatureId()).getImage().getFlippedCopy(flipped, false);
			skinGfx.setCenterOfRotation(50-SkinFeatureX*equipDir, 50-SkinFeatureY);
			skinGfx.setRotation(aniRotation);
			skinGfx.draw(x + getSkinFeatureX()*equipDir*scaleX - 25*scaleX, y + getSkinFeatureY()*scaleY - 25*scaleY, scaleX*100, scaleY*100); 
		}
		if(getMouthFeatureId() > 0){
			Image mouthGfx = ImageResource.getSprite("items/item"+getMouthFeatureId()).getImage().getFlippedCopy(flipped, false);
			mouthGfx.setCenterOfRotation(50-MouthFeatureX*equipDir, 50-MouthFeatureY);
			mouthGfx.setRotation(aniRotation);
			mouthGfx.draw(x + getMouthFeatureX()*equipDir*scaleX - 25*scaleX, y + getMouthFeatureY()*scaleY - 25*scaleY, scaleX*100, scaleY*100); 
		}
		if(getAccessoriesId() > 0){
			Image accessoriesGfx = ImageResource.getSprite("items/item"+getAccessoriesId()).getImage().getFlippedCopy(flipped, false);
			accessoriesGfx.setCenterOfRotation(50-AccessoriesX*equipDir, 50-AccessoriesY);
			accessoriesGfx.setRotation(aniRotation);
			accessoriesGfx.draw(x + getAccessoriesX()*equipDir*scaleX - 25*scaleX, y + getAccessoriesY()*scaleY - 25*scaleY, scaleX*100, scaleY*100); 
		}
	}
