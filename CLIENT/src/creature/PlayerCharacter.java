...
public class PlayerCharacter extends Creature {
  ...
  public void draw(Graphics g, int centerX, int centerY) {
    ...
    if (!Dead && !hidden) {
      ...
      if (BlueSaga.INPUT.getAbsoluteMouseX() > cornerX
          && BlueSaga.INPUT.getAbsoluteMouseY() > cornerY
          && BlueSaga.INPUT.getAbsoluteMouseX() < cornerX + SizeWidth * 50
          && BlueSaga.INPUT.getAbsoluteMouseY() < cornerY + SizeHeight * 50) {
        mouseOver = true;
      }
      ...
      if (!cloacked && mouseOver) {
        ...
        if (getPkMarker() > 0) {
          ImageResource.getSprite("gui/battle/pkmarker" + getPkMarker())
              .draw(nameX - 20, cornerY - 26);
        }

        // Draw bounty rank
        if (getBountyRank() > 0) {
          ImageResource.getSprite("gui/pvp/ranks/" + getBountyRank())
              .draw(nameX + nameWidth + 5, cornerY - 24);
          /*
      ...
      if (getDBId() == BlueSaga.playerCharacter.getDBId()) {
        if (BlueSaga.logoutTimeItr > 0) {
          ImageResource.getSprite("gui/emoticons/sweat_drop")
              .getAnimation()
              .getCurrentFrame()
              .getFlippedCopy(lookRight, false)
              .draw(
                  cornerX + (MyEquipHandler.getHeadX() + 13) * equipDir,
                  cornerY + MyEquipHandler.getHeadY() + 10);
        }
      }
    }
  }
  ...
  public void drawRestingFade() {
    ...
    if (RestingFadeAlpha > 0) {
      ImageResource.getSprite("effects/sleep_fade")
          .draw(0, 0, new Color(255, 255, 255, RestingFadeAlpha));
    }
  }
    ...
    for (int i = 0; i < Quests.size(); i++) {
      ...
      if (Quests.get(i).getType().equals("Kill X creature X")) {
        ...
        if (Quests.get(i).getTargetType().equals("Creature")) {
          rs =
              gameDB.askDB(
                  "select Kills from user_kills where UserId = "
                      + UserId
                      + " and CreatureId = "
          ...
          if (rs.next()) {
            ...
            if (Quests.get(i).getStatus() == 0) {
              if (rs.getInt("Kills") >= Quests.get(i).getTargetNumber()) {
                ...
                gameDB.updateDB(
                    "update quest_data set completed = 1 where QuestId = "
                        + Quests.get(i).getId()
                        + " and UserId = "
                        + UserId);
              }
