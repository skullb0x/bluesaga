...
public class Npc extends Creature {
	...
	private int AggroType; // 0 = not moving and no aggro, 1 = moving and no aggro, 2 = moving and aggro, 3 = npc quest/shop	
	...
	public void draw(Graphics g, int centerX, int centerY){
		...
		if(getSpecialType() > 0 && !isEpic() && !Dead){
			specialC = new Color(getSpecialColor().getRed(),getSpecialColor().getGreen(),getSpecialColor().getBlue(),150);
			ImageResource.getSprite("effects/fx_special_glow").draw(cornerX - 12 - ((SizeWidth-1)*20), cornerY - 12 - ((SizeHeight-1)*35), Math.round(sizeWidthF*75),Math.round(sizeHeightF*75),specialC);
		}
