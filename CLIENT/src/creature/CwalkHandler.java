...
public class CwalkHandler {
	...
	public void move(){
		...
		if((Math.round(moveX) == 0 && Math.round(moveY) == 0) || walkItr < 0){
        	...
        	if(ScreenHandler.SCREEN_TILES.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()) != null){
        		ScreenHandler.SCREEN_TILES.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()).showDust(posX, posY);
        		
        		String tileType = ScreenHandler.SCREEN_TILES.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()).getType();
        		...
        		if(ScreenHandler.SCREEN_TILES.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()) != null){
        			if(ScreenHandler.SCREEN_TILES.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()).getName().contains("path")){
        				tileType = "path";
        				showParticles = false;
        			}
        			
        			if(ScreenHandler.SCREEN_TILES.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()).getName().contains("carpet")){
        				carpet = true;
        			}
        			
        			if(ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()) != null){
        				if(ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()).getObject().getName().contains("bridge")){
        					showParticles = false;
        					bridge = true;
        				}
        			}
        			if(showParticles){
        				ScreenHandler.SCREEN_TILES.get(MyCreature.getX()+","+MyCreature.getY()+","+MyCreature.getZ()).showDust(MyCreature.getX()*ClientSettings.TILE_SIZE + 25, MyCreature.getY()*ClientSettings.TILE_SIZE + 40);
        			}
        		}
