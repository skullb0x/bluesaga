...
public class Creature {
	...
	public void useAbilityAnimate(int graphicsNr, Color useColor) {
		...
		timerAbility.schedule( new TimerTask(){
	        @Override
			public void run() {
	        	useAbilityAnimate = false;
	        }
		...
		if(!hidden){
			...
			if(ShowLevelUp || ShowLevelDown){
				if(animationUseAbility != null){
					animationUseAbility.draw(cornerX, cornerY-sizeHeightF*25.0f - (sizeWidthF-1)*50.0f - 5,new Color(255,255,135,200));
				}
			}else if(useAbilityAnimate){
				animationUseAbility.updateNoDraw();
				Image useAniImage = animationUseAbility.getCurrentFrame().getScaledCopy((int) (50*sizeWidthF), (int) (100*sizeHeightF));
				
				useAniImage.draw(cornerX, cornerY-sizeHeightF*25.0f - (sizeWidthF-1)*50.0f - 5, abilityColor);
			}
			...
			if(!Dead && !hasStatusEffect(13) && !hasStatusEffect(40)){
				...
				Shadow.getAnimation().getImage(framenr).draw(cornerX,shadowY,sizeWidthF*50,50, new Color(255,255,255,255));
			}
			...
			if(Dead){
				animation.getImage(0).getFlippedCopy(lookRight, false).drawFlash(cornerX, cornerY - Math.round(animationDeathY), SizeWidth*50, SizeHeight*50, new Color(255,255,255,200-Math.round(animationDeathY)*3));
			}else{
				...
				int transformFixY =  Math.round(- (sizeHeightF-1)*25 + sizeHeightF*25 - (25*sizeHeightF));
				
				// Draw equipment behind player
				if(!cloaked){
					if(!showBack){
						MyEquipHandler.drawBack(cornerX,cornerY+idleY + transformFixY, lookRight, aniSpin, scaleX, scaleY);
					}else{
						getCustomization().draw(cornerX, cornerY+idleY + transformFixY, lookRight, aniSpin, scaleX, scaleY);
						MyEquipHandler.drawFront(cornerX,cornerY + idleY + transformFixY,lookRight, aniSpin, scaleX, scaleY);
					}
				}
				...
				if(!cloaked){
					...
					Image transformedSprite = creatureSprite.getScaledCopy((int) (50*sizeWidthF), (int) (50*sizeHeightF));
					...
					// ABILITY SPRITE
					transformedSprite.draw(Math.round(cornerX), Math.round(cornerY) + transformFixY , animationColor); 
				}else if(hasStatusEffect(42)){
					ImageResource.getSprite("effects/flashstep").draw(Math.round(cornerX), Math.round(cornerY) + 10 + transformFixY);
				}else if(hasStatusEffect(40)){
					ImageResource.getSprite("objects/gathering/oranga_open").draw(Math.round(cornerX), Math.round(cornerY) + 10 + transformFixY);
				}
				...
				if(!cloaked){
					if(showBack){
						MyEquipHandler.drawHeadItem(cornerX,cornerY+idleY + transformFixY,lookRight, aniSpin, scaleX, scaleY);
						MyEquipHandler.drawBack(cornerX,cornerY+idleY + transformFixY, lookRight, aniSpin, scaleX, scaleY);
					}else{
						getCustomization().draw(cornerX, cornerY+idleY + transformFixY, lookRight, aniSpin, scaleX, scaleY);
						MyEquipHandler.drawFront(cornerX, cornerY+idleY + transformFixY, lookRight, aniSpin, scaleX, scaleY);
			...
			if(useAbilityAnimate){
				if(useAbilityGraphicsNr > 0){
					ImageResource.getSprite("abilities/ability_icon"+useAbilityGraphicsNr).drawCentered(centerX, cornerY);
				}
			}
	...
	public void addDamageLabel(String labelText, Color newColor, int graphicsNr){
		damageLabels.put(damageLabelId, new DamageLabel(labelText,newColor,X*ClientSettings.TILE_SIZE+25,Y*ClientSettings.TILE_SIZE+(SizeHeight-1)*25, graphicsNr));
		damageLabelId++;
		if(damageLabelId > 50){
			damageLabelId = 0;
		}
	...
	public void die() {
		...
		timerDie.schedule( new TimerTask(){
	        @Override
			public void run() {
	        	damageLabels.clear();
	    		dieHide();
	...
	public void updateItemCoordinates(){
	  if(MonsterHandler.creatureDefinitions.get(CreatureId) != null){
		    ...
		    getCustomization().setMouthFeatureX(MonsterHandler.creatureDefinitions.get(CreatureId).getCustomization().getMouthFeatureX());
		    getCustomization().setMouthFeatureY(MonsterHandler.creatureDefinitions.get(CreatureId).getCustomization().getMouthFeatureY());
	
		    getCustomization().setAccessoriesX(MonsterHandler.creatureDefinitions.get(CreatureId).getCustomization().getAccessoriesX());
		    getCustomization().setAccessoriesY(MonsterHandler.creatureDefinitions.get(CreatureId).getCustomization().getAccessoriesY());
	
		    getCustomization().setSkinFeatureX(MonsterHandler.creatureDefinitions.get(CreatureId).getCustomization().getSkinFeatureX());
		    getCustomization().setSkinFeatureY(MonsterHandler.creatureDefinitions.get(CreatureId).getCustomization().getSkinFeatureY());
	  }
	}
