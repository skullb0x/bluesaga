...
public class Projectile {
  ...
  public Projectile(
    ...
    ResultSet projectileInfo =
        BlueSaga.gameDB.askDB("select * from projectile where Id = " + ProjectileId);
    ...
    try {
      if (projectileInfo.next()) {
        ...
        myEmitter = null;
        ResultSet emitterInfo =
            BlueSaga.gameDB.askDB(
                "select Name from emitter where Id = " + projectileInfo.getInt("EmitterId"));
        if (emitterInfo.next()) {
          myEmitter =
              ScreenHandler.myEmitterManager.SpawnEmitter(
                  originX * ClientSettings.TILE_SIZE,
                  originY * ClientSettings.TILE_SIZE,
                  emitterInfo.getString("Name"));
        }
        emitterInfo.close();
        ...
        Animated =
            ImageResource.getSprite("projectiles/" + projectileInfo.getString("GfxName"))
                .isAnimated();
        ...
        if (effectId > 0) {
          ...
          effect =
              ImageResource.getSprite("projectiles/projectile_effect" + effectId)
                  .getAnimation()
                  .copy();
  ...
  public void draw(int x, int y) {
    ...
    if (Active) {
      ...
      if (myEmitter != null) {
        myEmitter.SetPosition(
            Math.round(OriginX * ClientSettings.TILE_SIZE + moveX + 25),
            Math.round(-OriginY * ClientSettings.TILE_SIZE + moveY + 25));
      }

      // REACH GOAL
      if (OriginX + Math.round(moveX / ClientSettings.TILE_SIZE) == goalX
          && OriginY - Math.round(moveY / ClientSettings.TILE_SIZE) == goalY) {
        Active = false;
      }
    }
  }
