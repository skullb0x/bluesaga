...
public class ScreenHandler {
  ...
  public static HashMap<String, ScreenObject> SCREEN_OBJECTS_WITH_ID =
      new HashMap<String, ScreenObject>();
  ...
  public static void draw(Graphics g, GameContainer app) {
    ...
    if (getActiveScreen() == ScreenType.LOADING || getActiveScreen() == ScreenType.ERROR) {
      ...
      for (int x = BlueSaga.playerCharacter.getX() - ClientSettings.TILE_HALF_W;
          x < BlueSaga.playerCharacter.getX() + ClientSettings.TILE_HALF_W + 1;
          x++) {
        for (int y = BlueSaga.playerCharacter.getY() - ClientSettings.TILE_HALF_H;
            y < BlueSaga.playerCharacter.getY() + ClientSettings.TILE_HALF_H + 1;
            y++) {
      ...
      if (Gui.USE_ABILITY) {
        if (Gui.getActionBar().getSelectedAbility() != null) {
          ...
          if (!AoE.equals("None")) {
            ...
            g.setColor(
                new Color(
                    activeAbility.getColor().getRed(),
                    activeAbility.getColor().getGreen(),
                    activeAbility.getColor().getBlue(),
                    100));
            for (String dXY_info : AoE_info) {
              ...
              g.fillRect(
                  renderPosX, renderPosY, ClientSettings.TILE_SIZE, ClientSettings.TILE_SIZE);
            }
          }
        }
      }
        ...
        if (INPUT.isMousePressed(1)) {
          ...
          if (!clickedGUI) {
            ...
            if (so != null) {
              ...
              if (Container != null) {
                if (Container.getName().contains("crafting")
                    || Container.getName().contains("gathering")
                    || Container.getName().contains("container")) {
                  if (Gui.ContainerWindow.isOpen()) {
                    Gui.ContainerWindow.close();
                  }
                  WalkHandler.OPEN_CONTAINER = true;
                  WalkHandler.OPEN_CONTAINER_ID =
                      Container.getName()
                          + ","
                          + tileX
                          + ","
                  ...
                  WalkHandler.findPath(
                      tileX, tileY, BlueSaga.playerCharacter.getZ(), "PlayerAttack");
          ...
          if (getActiveScreen() == ScreenType.WORLD) {
            ...
            if (!MapHandler.FADED_SCREEN) {
              ...
              if ((INPUT.isKeyDown(Input.KEY_S) && INPUT.isKeyDown(Input.KEY_A))
                  || (INPUT.isKeyDown(Input.KEY_DOWN) && INPUT.isKeyDown(Input.KEY_LEFT))) {
                dirX = -1;
                dirY = 1;
              } else if ((INPUT.isKeyDown(Input.KEY_S) && INPUT.isKeyDown(Input.KEY_D))
                  || (INPUT.isKeyDown(Input.KEY_DOWN) && INPUT.isKeyDown(Input.KEY_RIGHT))) {
                dirX = 1;
                dirY = 1;
              } else if ((INPUT.isKeyDown(Input.KEY_W) && INPUT.isKeyDown(Input.KEY_A))
                  || (INPUT.isKeyDown(Input.KEY_UP) && INPUT.isKeyDown(Input.KEY_LEFT))) {
                dirX = -1;
                dirY = -1;
  ...
  public static void updateCamera(float elapsedTime) {
    float cameraX =
        -BlueSaga.playerCharacter.getX() * ClientSettings.TILE_SIZE
            - BlueSaga.playerCharacter.MyWalkHandler.getMoveX();
    float cameraY =
        -BlueSaga.playerCharacter.getY() * ClientSettings.TILE_SIZE
            - BlueSaga.playerCharacter.MyWalkHandler.getMoveY();
