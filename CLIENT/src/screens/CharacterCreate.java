...
public class CharacterCreate {
	...
	public static void init(GameContainer app){
		...
		createButton = new Button(LanguageUtils.getString("ui.character_create.create_character").toUpperCase(),410,560,null);
	...
	public static void load(String createData){
		...
		for(String cData: creatureData){
			...
			CreatureButtons.add(new Button("",creatureButtonX + i%9*100, startY + 195 ,75, 100, null));
	...
	public static void draw(Graphics g, GameContainer app){
		...
		new_name.setLocation(startX + 265 - Font.size12bold.getWidth(new_name.getText())/2, posY+12);
		...
		*/
		...
		if(createState.equals("choose creature") || createState.equals("save info") || createState.equals("saving")){
			...
			for(int j = 0; j < CreatureButtons.size(); j++){
				...
				Creatures.get(j).draw(g, CreatureButtons.get(j).getX()+40,CreatureButtons.get(j).getY()+35,null);
				...
				g.drawString(Creatures.get(j).getName(), CreatureButtons.get(j).getX()+38 - textWidth/2,CreatureButtons.get(j).getY()+70);
			...
			for(int j = 0; j < ClassButtons.size(); j++){
				...
				int classId = j+1;
				ImageResource.getSprite("gui/menu/icon_class"+classId).draw(ClassButtons.get(j).getX()+ 15, ClassButtons.get(j).getY() + 15);
			}
	...
	public static String getCreateInfo(){
		...
		return new_name.getText()+";"+Creatures.get(selectedCreatureIndex).getCreatureId()+";"+classId;
	}
			...
			if(INPUT.isMousePressed(0)){
	 			if(selectFamily(INPUT.getAbsoluteMouseX(),INPUT.getAbsoluteMouseY())){
	 				...
	 				Sfx.play("gui/menu_confirm2");
	 					...
	 					&& selectedCreatureIndex != 100 && selectedClassIndex != 100){
	 				Sfx.play("gui/menu_confirm2");
