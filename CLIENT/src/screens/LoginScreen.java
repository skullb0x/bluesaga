...
public class LoginScreen {
  ...
  public static void init(GameContainer app, int x, int y) {
    ...
    ResultSet loginMail =
        BlueSaga.gameDB.askDB("select GameValue from option where GameOption = 'Mail'");
    try {
      if (loginMail.next()) {
        if (loginMail.getString("GameValue") != null
            && !loginMail.getString("GameValue").equals("")) {
    ...
    LoginButton =
        new Button(
            LanguageUtils.getString("ui.login.enter").toUpperCase(),
            410,
            360,
            Font.size12.getWidth(LanguageUtils.getString("ui.login.enter")) + 60,
            35,
            null);
    RegisterButton =
        new Button(
            LanguageUtils.getString("ui.login.create").toUpperCase(),
            398,
            500,
            Font.size12.getWidth(LanguageUtils.getString("ui.login.create")) + 60,
            35,
            null);
    ForgotButton =
        new Button(
            LanguageUtils.getString("ui.login.forgot"),
            418,
            550,
            Font.size12.getWidth(LanguageUtils.getString("ui.login.forgot")) + 60,
            35,
            null);
  }
    ...
    if (ScreenHandler.getActiveScreen() == ScreenType.LOGIN) {
      ...
      if (retryCountdown > 0) {
        g.drawString(
            statusMessage
                + ", "
                + LanguageUtils.getString("ui.login.retry")
                + " "
                + retryCountdown
                + "s",
            X
                + 190
                - Font.size12.getWidth(
                        statusMessage
                            + ", "
                            + LanguageUtils.getString("ui.login.retry")
                            + " "
    ...
    if (!ClientSettings.DEV_MODE) {
      BlueSaga.gameDB.updateDB(
          "update option set GameValue = '"
              + mail_field.getText()
              + "' where GameOption like 'Mail'");
    }

    // ENCODE PASSWORD
    String encryptedPassword =
        Encryption.encryptPassword(mail_field.getText(), password_field.getText());
