...
public class CharacterSelection {
	...
	public static void init(GameContainer app){
		DeleteCharacterButton = new Button(LanguageUtils.getString("ui.character_selection.delete").toUpperCase(), 410,520, null);
		ChangePasswordButton = new Button(LanguageUtils.getString("ui.character_selection.change_password").toUpperCase(), 410,570, null);
	...
	public static void load(String characterInfo){
		...
		if(!characterInfo.equals("None")){
			...
			for(String char_info: characters_info){
				...
				Levels.add(level);
				Deleted[i] = deleted;
				Buttons.add(new Button("",startX + i%4*buttonSpace, (int) (startY+buttonSpace*Math.floor(i/4))+70,buttonWidth,buttonWidth, null));
				i++;
			}
		}
		while(i < 8){
	...
	public static void draw(Graphics g, GameContainer app){
		...
		if(ScreenHandler.getActiveScreen() == ScreenType.CHARACTER_SELECT 
			...
			for(int i = 0; i < 8; i++){
				...
				if(drawChar){
					...
					if(i < Characters.size()){
						...
						g.drawString(Names.get(i), startX+i%4*buttonSpace + 60 - Font.size12.getWidth(Names.get(i)) / 2, (int) (startY+buttonSpace*Math.floor(i/4))+85);
						g.setColor(BlueSagaColors.WHITE);
						g.drawString("LVL "+Levels.get(i), startX+i%4*buttonSpace+ 60 - Font.size12.getWidth("LVL "+Levels.get(i)) / 2, (int) (startY+buttonSpace*Math.floor(i/4))+165);
					
						if(Deleted[i] > -1){
							Characters.get(i).draw(g, startX+i%4*buttonSpace+ 60, (int) (startY+buttonSpace*Math.floor(i/4)) + 130,null);
							if(Buttons.get(i).isClicked(mouseX, mouseY)){
								g.setColor(new Color(BlueSagaColors.YELLOW.getRed(),BlueSagaColors.YELLOW.getGreen(),BlueSagaColors.YELLOW.getBlue(),200));
							}else{
								g.setColor(new Color(BlueSagaColors.RED.getRed(),BlueSagaColors.RED.getGreen(),BlueSagaColors.RED.getBlue(),200));
							}
						...
						if(ScreenHandler.getActiveScreen() == ScreenType.CHARACTER_DELETE){
							if(Deleted[i] == -1){
								if(Buttons.get(i).isClicked(mouseX, mouseY)){
									ImageResource.getSprite("gui/menu/delete_character").draw(startX+i%4*buttonSpace + 13, (int) (startY+buttonSpace*Math.floor(i/4)) + 80, new Color(255,255,255,200));
								}else{
									ImageResource.getSprite("gui/menu/delete_character").draw(startX+i%4*buttonSpace + 13, (int) (startY+buttonSpace*Math.floor(i/4)) + 80, new Color(255,255,255,100));
								}
							}else{
								if(Buttons.get(i).isClicked(mouseX, mouseY)){
									g.setColor(BlueSagaColors.BLACK);
								...
								g.drawString(LanguageUtils.getString("ui.character_selection.recover").toUpperCase(), startX+i%4*buttonSpace+ 30, (int) (startY+buttonSpace*Math.floor(i/4)) + 125);
							}
						}else{
							if(Deleted[i] > -1){
								if(Buttons.get(i).isClicked(mouseX, mouseY)){
								...
								g.drawString(LanguageUtils.getString("ui.character_selection.deleted_in").toUpperCase(), startX+i%4*buttonSpace+ 20, (int) (startY+buttonSpace*Math.floor(i/4)) + 117);
								
								int daysLeft = 3 - Deleted[i];
								g.drawString(daysLeft+" "+LanguageUtils.getString("ui.character_selection.days").toUpperCase(), startX+i%4*buttonSpace+ 35, (int) (startY+buttonSpace*Math.floor(i/4)) + 132);
							}
						}
						...
						g.drawString(LanguageUtils.getString("ui.character_selection.create").toUpperCase(), startX+i%4*buttonSpace+ 35, (int) (startY+buttonSpace*Math.floor(i/4)) + 122);
					}	
				}
			}
			...
			g.drawString(Names.get(CharacterSelectedIndex), charX + 27 - Font.size12.getWidth(Names.get(CharacterSelectedIndex)) / 2, charY-18);
			g.setColor(BlueSagaColors.YELLOW);
			g.drawString(Names.get(CharacterSelectedIndex), charX + 25 - Font.size12.getWidth(Names.get(CharacterSelectedIndex)) / 2, charY-20);
			
			g.setColor(new Color(0,0,0,100));
			g.drawString("LVL "+Levels.get(CharacterSelectedIndex), charX + 27 - Font.size12.getWidth("LVL "+Levels.get(CharacterSelectedIndex)) / 2, charY+72);
			g.setColor(BlueSagaColors.WHITE);
			g.drawString("LVL "+Levels.get(CharacterSelectedIndex), charX + 25 - Font.size12.getWidth("LVL "+Levels.get(CharacterSelectedIndex)) / 2, charY+70);
	...
	public static void keyLogic(Input INPUT){
		if(INPUT.isKeyPressed(Input.KEY_ESCAPE)){
			...
			if(INPUT.isMousePressed(0)){
	 			if(ScreenHandler.getActiveScreen() == ScreenType.CHARACTER_SELECT || ScreenHandler.getActiveScreen() == ScreenType.CHARACTER_DELETE){
		 			...
		 			if(clickedId == 0){
		 				...
		 				if(ScreenHandler.getActiveScreen() == ScreenType.CHARACTER_SELECT){
		 					...
		 					ScreenHandler.LoadingStatus = LanguageUtils.getString("ui.character_selection.request_data");
		 					ScreenHandler.setActiveScreen(ScreenType.LOADING);
		 			...
		 			if(DeleteCharacterButton.isClicked(INPUT.getAbsoluteMouseX(),INPUT.getAbsoluteMouseY())){
		 				if(ScreenHandler.getActiveScreen() == ScreenType.CHARACTER_DELETE){
		 					ScreenHandler.setActiveScreen(ScreenType.CHARACTER_SELECT);
			 				DeleteCharacterButton.setLabel(LanguageUtils.getString("ui.character_selection.delete"));
		 				}else{
		 					ScreenHandler.setActiveScreen(ScreenType.CHARACTER_DELETE);
		 					DeleteCharacterButton.setLabel(LanguageUtils.getString("ui.character_selection.cancel"));
		 				}
		 			}else if(ChangePasswordButton.isClicked(INPUT.getAbsoluteMouseX(),INPUT.getAbsoluteMouseY())){
	 				...
	 				if(!change_status.equals(LanguageUtils.getString("ui.character_selection.send")) && ChangePasswordButton.isClicked(INPUT.getAbsoluteMouseX(),INPUT.getAbsoluteMouseY())){
		 				...
		 				if(!new_password.getText().equals(new_password_repeat.getText())){
		 					...
		 					String old_password_enc = Encryption.encryptPassword(BlueSaga.client.getUserMail(), old_password.getText());
		 					String new_password_enc = Encryption.encryptPassword(BlueSaga.client.getUserMail(), new_password.getText());
		 					
		 					BlueSaga.client.sendMessage("changepassword", old_password_enc+";"+new_password_enc);
			...
			if(INPUT.isKeyPressed(Input.KEY_TAB) && ScreenHandler.getActiveScreen() == ScreenType.CHANGE_PASSWORD){
