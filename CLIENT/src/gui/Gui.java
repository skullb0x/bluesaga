...
public class Gui {
    ...
    if (ScreenHandler.getActiveScreen() == ScreenType.WORLD) {
      ...
      if (ScreenHandler.getActiveScreen() != ScreenType.LOGIN
        ...
        String mousePos[] =
            BlueSaga.WORLD_MAP
                .getClickedTile(
                    mouseX, mouseY, ScreenHandler.myCamera.getX(), ScreenHandler.myCamera.getY())
                .split(";");
        ...
        if (mouseTileX > 0 && mouseTileY > 0) {
          ...
          for (ScreenObject c : ScreenHandler.SCREEN_OBJECTS_DRAW) {
            if (c.getType().equals("Creature")) {
              ...
              if (!(c.getCreature().getX() == BlueSaga.playerCharacter.getX()
                ...
                if (mouseX > c.getCreature().getScreenPixelX() - 10
                    && mouseX
                        < c.getCreature().getScreenPixelX()
                            + c.getCreature().getSizeWidthF() * ClientSettings.TILE_SIZE
                            + 10
                    && mouseY > c.getCreature().getScreenPixelY() + 15 - 10
                    && mouseY
                        < c.getCreature().getScreenPixelY()
                            + c.getCreature().getSizeHeightF() * ClientSettings.TILE_SIZE
                            + 15
                            + 10) {
                  ...
                  } else if (ScreenHandler.SCREEN_TILES.get(
                          c.getCreature().getX()
                              + ","
                              + c.getCreature().getY()
                              + ","
          ...
          if (!USE_ABILITY && !USE_SCROLL && attackTarget != null) {
            ...
            if (ActiveAbility != null) {
              // CHECK RANGE
              if (Math.abs(mouseTileX - BlueSaga.playerCharacter.getX())
                      + Math.abs(mouseTileY - BlueSaga.playerCharacter.getY())
                  > ActiveAbility.getRange()) {
                // Not in range
                mouseAlpha = 100;
              }
            } else {
              ...
              && ScreenHandler.SCREEN_OBJECTS_WITH_ID.get(
                      mouseTileX + "," + mouseTileY + "," + mouseTileZ)
                  != null) {
            if (ScreenHandler.SCREEN_OBJECTS_WITH_ID
                    .get(mouseTileX + "," + mouseTileY + "," + mouseTileZ)
                    .getObject()
                != null) {
              if (!USE_ABILITY
  ...
  public void drawPlayerInfo(Graphics g, int x, int y, int mouseX, int mouseY) {
    ...
    g.setWorldClip(
        x + 5,
        y + 5 + 40 - BlueSaga.playerCharacter.getXPBarWidth(40),
        40,
        BlueSaga.playerCharacter.getXPBarWidth(40));
  ...
  public static void addRequest(String requestType, int requestId, String newMessage) {
    ...
    if (!duplicate) {
      RequestNotification newNotif =
          new RequestNotification(newMessage, requestType, requestId, NotificationsY);
      NotificationsY += 45 + newNotif.getNrTextLines() * 15;
      Notifications.add(newNotif);
    }
    Sfx.play("notifications/quest_updated");
        ...
        if (BlueSaga.playerCharacter.isResting()) {
          ...
          if (ScreenHandler.SCREEN_TILES.get(
            ...
            if (!ScreenHandler.SCREEN_TILES
                ...
                .getType()
                .equals("indoors")) {
              if (BlueSaga.playerCharacter.getStat("HEALTH_REGAIN") == 0
                  && BlueSaga.playerCharacter.getStat("MANA_REGAIN") == 0) {
                Gui.addMessage(
                    LanguageUtils.getString("messages.quest.need_eat_first"), BlueSagaColors.RED);
              }
            }
  ...
  public static boolean leftMouseClick(Input INPUT, boolean DoAction) {
    ...
    if (!clickedGUI && DoAction) {
      ...
      if (USE_ABILITY) {
        ...
        Ability ActiveAbility = ActionBar.getSelectedAbility();
        // CHECK RANGE
        if (Math.abs(tileX - BlueSaga.playerCharacter.getX())
                + Math.abs(tileY - BlueSaga.playerCharacter.getY())
            <= ActiveAbility.getRange()) {
          if (AbilityHandler.readyToUseAbility()) {
            BlueSaga.client.sendMessage(
                "use_ability", tileX + "," + tileY + "," + ActiveAbility.getAbilityId());
          }
        }
      } else if (USE_SCROLL) {
        if (USE_SCROLL_TYPE.equals("Ability")) {
          clickedGUI = true;
          BlueSaga.client.sendMessage(
              "use_scroll", tileX + "," + tileY + "," + USE_SCROLL_ID + "," + USE_SCROLL_LOCATION);
        }
      } else {
        if (MouseItem.getItem() != null) {
          ...
          if (ScreenHandler.SCREEN_TILES.get(
                  tileX + "," + tileY + "," + BlueSaga.playerCharacter.getZ())
              != null) {
            ScreenHandler.SCREEN_TILES
                .get(tileX + "," + tileY + "," + BlueSaga.playerCharacter.getZ())
                .showWalkTo();
          }
        }
      }
  ...
  public static boolean isInputActive() {
    if (BountyWindow.isOpen()
        || Chat_Window.isActive()
        || ItemSplitterWindow.isOpen()
        || (PlayersWindow.isOpen()
            && (PlayersWindow.getActiveView().equals("ADD FRIEND")
                || PlayersWindow.getActiveView().equals("ADD TO PARTY")))) {
      return true;
    }
    return false;
  }
  ...
  public void drawBounty(Graphics g, int x, int y) {
    if (SHOW_BOUNTY) {
      ...
      if (bountyOpacity < 255) {
        ...
        if (newBounty != oldBounty + bountyChange
            && oldBounty + bountyChange / Math.abs(bountyChange) > 0) {
          newBounty += bountyChange / Math.abs(bountyChange);
        } else {
          bountyTimer.schedule(
              new TimerTask() {
                @Override
                public void run() {
                  SHOW_BOUNTY = false;
      ...
      ImageResource.getSprite("gui/skulls/silver")
          .getImage()
          .draw(textX + Font.size30.getWidth(newBounty + "   ") / 2 + 40, y + 22);
      ...
      ImageResource.getSprite("gui/skulls/silver")
          .getImage()
          .draw(
              textX + Font.size30.getWidth(newBounty + "   ") / 2 + 40,
              y + 22,
              40,
              40,
              new Color(255, 255, 255, bountyOpacity));
