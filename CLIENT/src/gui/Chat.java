...
public class Chat {
		...
		if(isVisible()){
			...
			for(String channel: chatChannels){
				...
				if(chatChannelsColor.get(channel) != null){
					channelColor = new Color(chatChannelsColor.get(channel).getRed(),chatChannelsColor.get(channel).getGreen(),chatChannelsColor.get(channel).getBlue());
				}
				...
				if(NewChatMessage.get(channel)){
					if(NewMessageBlinkItr % 20 < 10){
						...
						g.setColor(new Color(channelColor.getRed(),channelColor.getGreen(),channelColor.getBlue(),100));
					}
				}else{
					if(ActiveChatChannel.equals(channel)){
						g.setColor(channelColor);
					}else{
						g.setColor(new Color(channelColor.getRed(),channelColor.getGreen(),channelColor.getBlue(),200));
					}
				}
			...
			int totalRows = rows_count.get(ActiveChatChannel)+rows_count.get("event")+rows_count.get("error");
			...
			for(int i = 0; i < rows_text.size();i++){
				...
				if(show){
					...
					if(!special_type.contains(rows_type.get(i)) && !rows_author.get(i).equals("error")  && !rows_author.get(i).equals("event")){
						
						if(!same_author){
							g.drawString(rows_author.get(i)+": ", textpos, drawY + height - 15 - (totalRows-nrRow)*17);
	...
	public void formatTextRow(String stringToFormat, String chatType, String author){
		...
		if(!special_type.contains(chatType) && !ActiveChatChannel.equals(chatType) && !chatType.equals("private")){
			NewChatMessage.put(chatType, true);
			NewMessageBlinkItr = 1;
		}
		...
		if(INPUT.isKeyPressed(Input.KEY_ENTER)){
			if(!isActive()){
				...
				if(chatText.length() > 0){
					...
					if(BlueSaga.playerCharacter.getAdminLevel() == 0 && ActiveChatChannel.equals("announce") && announce_cooldown_itr > 0){
						sendOk = false;
	...
	public boolean isClickedOn(int mouseX, int mouseY){
		...
		if(BigChat){
			if(mouseX > X && mouseX < Width+20 && mouseY > Y-295 && mouseY < ClientSettings.SCREEN_HEIGHT-20){
				clickedChat = true;
			}

		}else if(!BigChat){
			if(mouseX > X && mouseX < Width+20 && mouseY > Y && mouseY < ClientSettings.SCREEN_HEIGHT-20){
				clickedChat = true;
			}
		}
		return clickedChat;
	...
	public void addChatChannel(String type){
		if(!chatChannels.contains(type)){
			...
			if(type.equals("crew")){
				...
				chatChannelsColor.put(type, new Color(RandomUtils.getInt(100, 255),RandomUtils.getInt(100, 255),RandomUtils.getInt(100, 255)));
			}
