	...
	public void draw(Graphics g, int mouseX, int mouseY){
		if(myAbility != null){
			...
			if(myItem.getGraphicsNr() > 0){
				ImageResource.getSprite("items/skins/"+myItem.getGraphicsNr()).getImage().drawCentered(mouseX, mouseY);
			}else{
				ImageResource.getSprite("items/item"+myItem.getId()).getImage().drawCentered(mouseX, mouseY);
			}
		}
	}
