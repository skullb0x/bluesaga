...
public class PlayersWindow extends Window {
  ...
  public PlayersWindow(GameContainer app, int x, int y, int width, int height) {
    ...
    AddFriendButton =
        new Button(
            LanguageUtils.getString("ui.players_window.add_friend").toUpperCase(),
            20,
            height - 58,
            this);
    CancelButton =
        new Button(
            LanguageUtils.getString("ui.buttons.cancel").toUpperCase(), 170, 125, 80, 38, this);

    BackButton =
        new Button(
            LanguageUtils.getString("ui.buttons.back").toUpperCase(),
            170,
            height - 58,
            80,
            38,
    ...
    PartyButton =
        new Button(
            LanguageUtils.getString("ui.players_window.party").toUpperCase(),
            170,
            height - 58,
            80,
            38,
    ...
    if (isVisible()) {
      ...
      if (isFullyOpened()) {
        if (ACTIVE_VIEW.equals("LIST")) {
          ...
          g.drawString(
              LanguageUtils.getString("ui.players_window.players_online").toUpperCase() + ": ",
              X + 20 + moveX,
              Y + 20 + moveY);
          ...
          if (PlayerButtons1.getNrListItems() == 0) {
            g.drawString(
                LanguageUtils.getString("ui.players_window.no_friends"),
                X + 20 + moveX,
                Y + 60 + moveY);
          }
          ...
          g.drawString(
              LanguageUtils.getString("ui.players_window.enter_player_name"),
              X + 20 + moveX,
              Y + 60 + moveY);
          ...
          if (PlayerButtons1.getNrListItems() == 0) {
            g.drawString(
                LanguageUtils.getString("ui.players_window.no_party_active"),
                X + 20 + moveX,
                Y + 20 + moveY);
          } else {
            g.drawString(
          ...
          g.setFont(Font.size16);
          g.drawString(
              LanguageUtils.getString("ui.players_window.add_player_to_party").toUpperCase(),
              X + 20 + moveX,
              Y + 20 + moveY);
  ...
  public void keyLogic(Input INPUT) {
    if (!ACTIVE_VIEW.equals("ADD FRIEND")
        && !ACTIVE_VIEW.equals("ADD TO PARTY")
        && INPUT.isKeyPressed(Input.KEY_P)
        && !Gui.Chat_Window.isActive()) {
      toggle();
    }
  }
