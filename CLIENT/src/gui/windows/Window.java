...
public class Window {
  ...
  public void draw(GameContainer app, Graphics g, int mouseX, int mouseY) {
    if (isVisible()) {
      ...
      if (aniWidth > 0 || aniHeight > 0) {
        ...
        g.fillRect(
            Math.round((Width - aniWidth) / 2 + realX),
            Math.round((Height - aniHeight) / 2 + 8 + realY),
            Math.round(aniWidth),
            Math.round(aniHeight - 16));
        g.fillRect(
            Math.round((Width - aniWidth) / 2 + 8 + realX),
            Math.round((Height - aniHeight) / 2 + realY),
            Math.round(aniWidth - 16),
            Math.round(aniHeight));

        // LIGHTER COLOR
        g.setColor(BG_COLOR);
        g.fillRoundRect(
            Math.round(realX + (Width - aniWidth) / 2 + 4),
            Math.round(realY + (Height - aniHeight) / 2 + 4),
            Math.round(aniWidth - 8),
            Math.round(aniHeight - 8),
            5);

        ulCorner.draw(
            Math.round(realX + (Width - aniWidth) / 2),
            Math.round(realY + (Height - aniHeight) / 2),
            BORDER_COLOR);
        dlCorner.draw(
            Math.round(realX + (Width - aniWidth) / 2),
            Math.round(realY + (Height + aniHeight) / 2 - 12),
            BORDER_COLOR);

        urCorner.draw(
            Math.round(realX + Width - (Width - aniWidth) / 2 - 12),
            Math.round(realY + (Height - aniHeight) / 2),
            BORDER_COLOR);
        drCorner.draw(
            Math.round(realX + Width - (Width - aniWidth) / 2 - 12),
  ...
  public void leftMouseClick(Input INPUT) {
    if (isOpen()) {
      if (showCloseButton
          && INPUT.getAbsoluteMouseX() > X + moveX + Width - 35
