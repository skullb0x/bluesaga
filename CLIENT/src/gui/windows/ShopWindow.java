		...
		if(isVisible() && !BlueSaga.actionServerWait){
			if(clickedOn(mouseX,mouseY)){
				if(Gui.MouseItem.getItem() != null){
					// SELL ITEM
					BlueSaga.client.sendMessage("sell", "mouse,"+Gui.MouseItem.getItem().getUserItemId()+"");
				...
				for(int j = 0; j < nrBoxH; j++){
					...
					if(Boxes[i][j].isSelected(mouseX, mouseY, moveX, moveY)){
						...
						if(Boxes[i][j].getItem() != null){
							BlueSaga.client.sendMessage("buy", Boxes[i][j].getContentType()+";"+Boxes[i][j].getItem().getId());
							BlueSaga.actionServerWait = true;
						}else if(Boxes[i][j].getAbility() != null){
							BlueSaga.client.sendMessage("buy", Boxes[i][j].getContentType()+";"+Boxes[i][j].getAbility().getAbilityId());
							BlueSaga.actionServerWait = true;
						}
						break;
					}
							...
							if(!Boxes[i][j].isEmpty()){
								...
								if(Boxes[i][j].getItem() != null){
									...
									if(selectedAbilityForInfo != null){
										if(selectedAbilityForInfo.getAbilityId() != Boxes[i][j].getAbility().getAbilityId()){
											showInfo = true;
										}
									}else{
										showInfo = true;
									...
									if(showInfo){
										...
										SelectedInfoBox = new ItemInfoBox(infoBoxX, Y + j*50 + moveY,1,1);
										BlueSaga.client.sendMessage("ability_info", "shop;"+selectedAbilityForInfo.getAbilityId());
									}	
								}
