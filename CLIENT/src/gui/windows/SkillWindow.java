			...
			if(isFullyOpened()){
				...
				if(SkillType.equals("Job")){
					...
					for(Skill s: BlueSaga.playerCharacter.getSkills().values()){
						if(s != null){
							ImageResource.getSprite("skills/icon"+s.getId()).draw(X + 20 + moveX, Y + 65 + moveY + (i)* 30);
							ImageResource.getSprite("gui/world/meter_bg").draw(X + 50 + moveX, Y + 70 + moveY + (i)* 30);

							g.setWorldClip(X + 50 + moveX, Y + 70 + moveY + (i)* 30,s.getSPBarWidth(107),20);
							ImageResource.getSprite("gui/world/xp_small_meter").draw(X + 55 + moveX, Y + 74 + moveY + (i)* 30);
							g.clearWorldClip();

							g.drawString(s.getName() + " - Lvl "+s.getLevel(), X + 180 + moveX, Y + 73 + moveY + (i)* 30);
							i++;
						}	
					}	
				}else{
					...
					for(BaseClass playerClass: BlueSaga.playerCharacter.getPlayerClasses().values()){
						if(playerClass.available){
							ImageResource.getSprite("classes/icon"+playerClass.id).draw(X + 20 + moveX, Y + 65 + moveY + (i)* 30);
							ImageResource.getSprite("gui/world/meter_bg").draw(X + 50 + moveX, Y + 70 + moveY + (i)* 30);
							
							g.setWorldClip(X + 50 + moveX, Y + 70 + moveY + (i)* 30,playerClass.getXPBarWidth(107),20);
							if(playerClass.id < 4){
								ImageResource.getSprite("gui/world/skill_meter").draw(X + 55 + moveX, Y + 74 + moveY + (i)* 30);
							}else{
								ImageResource.getSprite("gui/world/xp_small_meter").draw(X + 55 + moveX, Y + 74 + moveY + (i)* 30);
							}
							g.clearWorldClip();
							
							g.drawString(playerClass.name + " - Lvl "+playerClass.level, X + 180 + moveX, Y + 73 + moveY + (i)* 30);
							i++;
						}
					}	
				}
