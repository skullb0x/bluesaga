								...
								if(itemBoxes.get(i+","+j).getItem() != null){
									...
									if(showInfo){
										...
										if(getName().equals("Personal Chest")){
											BlueSaga.client.sendMessage("item_info", "personalchest;"+selectedItemForInfo.getUserItemId());
										}else if(getName().equals("Closet")){
											BlueSaga.client.sendMessage("item_info", "closet;"+selectedItemForInfo.getId());
										}else{
											BlueSaga.client.sendMessage("item_info", "container;"+getContainerX()+","+getContainerY()+","+getContainerZ()+";"+i+","+j);
										}
									}
								}else{
									selectedItemForInfo = null;
							...
							if(!BlueSaga.actionServerWait){
								...
								//	BlueSaga.actionServerWait = true;
									...
									if(Name.equals("Personal Chest")){
										...
										// ItemId; posX; posY
										BlueSaga.client.sendMessage("moveitem", Gui.MouseItem.getItem().getId()+";"+i+";"+j+";1");
										Gui.MouseItem.clear();
										BlueSaga.actionServerWait = true;
									}else{
								...
								}else if(!Id.startsWith("Shop") && Gui.MouseItem.isEmpty() && !itemBoxes.get(i + "," + j).isEmpty()){
					...
					if(itemBoxes.get(i + "," + j).isSelected(mouseX, mouseY, moveX, moveY)){
						...
						if(Name.equals("Closet")){
							if(!itemBoxes.get(i + "," + j).isEmpty()){
								BlueSaga.client.sendMessage("set_skin", ""+itemBoxes.get(i + "," + j).getItem().getId());
							}
						}else {
							if(!itemBoxes.get(i + "," + j).isEmpty()){
								BlueSaga.client.sendMessage("fastloot", ""+ContainerX+","+ContainerY+","+ContainerZ+","+i+","+j);
							}
						}
					}
					if(clickedBox){
