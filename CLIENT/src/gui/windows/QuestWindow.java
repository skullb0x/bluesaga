...
public class QuestWindow extends Window {
		...
		if(isVisible()){
			...
			if(isFullyOpened()){
				...
				if(State.equals("List")){
					...
					g.drawString(LanguageUtils.getString("ui.status.loading"), X + 20 + moveX, Y + 20 + moveY);
				}else if(State.equals("ShowQuest")){
					g.setColor(new Color(255,255,255,255));
					g.drawString(Quests.get(selectedQuest).getName(), X+20+moveX, Y+20+moveY);
					
					g.drawString("("+LanguageUtils.getString("ui.quest.rec_level")+": "+Quests.get(selectedQuest).getLevel()+"+ )", X+20+moveX, Y+50+moveY);
					
					if(Quests.get(selectedQuest).getStatus() == 1){
						g.setColor(BlueSagaColors.WHITE);
						g.drawString(LanguageUtils.getString("ui.quest.status_accepted"), X+20 + moveX, Y+80+moveY);
					}else if(Quests.get(selectedQuest).getStatus() == 2){
						g.setColor(BlueSagaColors.YELLOW);
						g.drawString(LanguageUtils.getString("ui.quest.status_get_reward"), X+20 + moveX, Y+80+moveY);
					}
					g.setColor(BlueSagaColors.WHITE);
	...
	public void showQuest(String questText){
		Quests.get(selectedQuest).setDescription(LanguageUtils.getString("quests."+Quests.get(selectedQuest).getId()+".description"));
		State = "ShowQuest";
	}
