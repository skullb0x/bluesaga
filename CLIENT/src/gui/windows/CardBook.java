            ...
            if (Boxes[i][j].isSelected(mouseX, mouseY, moveX, moveY)) {
              ...
              if (Boxes[i][j].getItem() != null) {
                ...
                if (selectedItemForInfo != null) {
                  if (selectedItemForInfo.getUserItemId()
                      != Boxes[i][j].getItem().getUserItemId()) {
                    showInfo = true;
                  }
                } else {
                  showInfo = true;
                ...
                if (showInfo) {
                  ...
                  if (Gui.ShopWindow.isFullyOpened()) {
                    BlueSaga.client.sendMessage(
                        "item_info", "invshop;" + selectedItemForInfo.getUserItemId());
                  } else {
                    BlueSaga.client.sendMessage(
                        "item_info", "inv;" + selectedItemForInfo.getUserItemId());
                  }
                }
              } else {
                selectedItemForInfo = null;
        ...
        for (int j = 0; j < sizeWidth; j++) {
          ...
          if (Boxes[i][j].isSelected(mouseX, mouseY, moveX, moveY)) {
            ...
            if (!Gui.MouseItem.isEmpty()
                && Gui.MouseItem.getItem() != null
                && Gui.MouseItem.getItem().getType().equals("Collector Card")) {
              // ADD CARD TO CARDBOOK
              // ItemId, posX, posY, pageNumber
              BlueSaga.client.sendMessage(
                  "card_place",
                  Gui.MouseItem.getItem().getId() + "," + Boxes[i][j].getCardNumber());
              Gui.MouseItem.clear();
