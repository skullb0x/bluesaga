...
public class NpcDialog extends Window {
	...
	public void showQuestInfo(int questId, String questType, String questMessage, int questStatus){
		...
		if(questType.equals("Learn Class")){
			CancelButton = new Button(LanguageUtils.getString("ui.buttons.no_thanks").toUpperCase(), 20, Height - 40, 104, 35, this);
		}else if(questStatus == 0 && !questType.equals("Story") && !questType.equals("Instructions")){
			CancelButton = new Button(LanguageUtils.getString("ui.buttons.no_way").toUpperCase(), 250, Height - 40, 130, 35, this);
		}else if(questStatus == 1){
			CancelButton = new Button(LanguageUtils.getString("ui.buttons.on_my_way").toUpperCase(), 160, Height - 40, 200, 35, this);
		}else if(questStatus > 1 || questType.equals("Story") || questType.equals("Instructions")){
			CancelButton = new Button(LanguageUtils.getString("ui.buttons.end_conversation").toUpperCase(), 150, Height - 40, 220, 35, this);
		}
			...
			if(isFullyOpened()){
				...
				if(MenuState == 0){
					...
					if(MenuState < QuestMessage.size()){
						...
						if(SelectedQuest.getType().equals("Story") || SelectedQuest.getType().equals("Instructions")){
				...
				if(MenuState == 0){
					...
					if(selectedIndex < 9999){
						if(List.getListItem(selectedIndex).getQuest() != null){
							...
							// BOUNTY
							// TIMER!!!
							bountyWindowTimer.schedule( new TimerTask(){
						        @Override
								public void run() {
						      	close();
