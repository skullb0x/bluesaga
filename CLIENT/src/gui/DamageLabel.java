...
public class DamageLabel {
  ...
  public void draw(Graphics g, int playerX, int playerY) {
    ...
    int labelX =
        playerX
            - ((playerX - cameraX) - X)
            - Math.round((float) LabelFont.getWidth(LabelText) / 2)
            - 25; // - cameraX;
    ...
    if (AbilityId > 0) {
      ImageResource.getSprite("abilities/ability_icon" + AbilityId)
          .draw(labelX - 30, labelY - 15, new Color(255, 255, 255, Alpha));
    }
