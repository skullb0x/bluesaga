...
public class Streak {
	...
	public void Update(float aElapsedTime) {
		...
		if (myStreakDots.size() <= 0) {
			...
			diffDist.x = myPosition.x
					- myStreakDots.get(myStreakDots.size() - 1).myPosition.x;
			diffDist.y = myPosition.y
					- myStreakDots.get(myStreakDots.size() - 1).myPosition.y;
	...
	public void Render(Graphics g, Camera aCamera) {
		...
		for (int index = 0; index < myStreakDots.size(); ++index) {
			...
			float firstRenderPosX = firstPos.x + aCamera.getfX();
			float firstRenderPosY = firstPos.y + aCamera.getfY();
			float secondRenderPosX = secondPos.x + aCamera.getfX();
			float secondRenderPosY = secondPos.y + aCamera.getfY();
			g.drawGradientLine(firstRenderPosX, firstRenderPosY, firstColor, secondRenderPosX, secondRenderPosY, secondColor);
		}
