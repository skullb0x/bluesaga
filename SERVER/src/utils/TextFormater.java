...
public class TextFormater {
  ...
  public static void formatBonusInfo(StringBuilder sb, String label, int statValue) {
    if (statValue != 0) {
      if (statValue < 0) {
        sb.append("190,60,60;").append(label).append(statValue).append('/');
      } else {
        sb.append("0,180,0;").append(label).append('+').append(statValue).append('/');
      }
    }
  }

  public static void formatReqInfo(
      StringBuilder sb, String label, int statValue, int characterValue) {
    if (statValue != 0) {
