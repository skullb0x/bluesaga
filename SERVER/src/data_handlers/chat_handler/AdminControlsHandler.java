...
public class AdminControlsHandler extends Handler {
	...
	private static boolean levelTwoControls(Client client, String chatText){
		...
		if(chatText.toLowerCase().startsWith("/changename ")){
			...
			if(nameInfo.length > 1){
				...
				ResultSet nameCheck = Server.userDB.askDB("select Id from user_character where lower(Name) = '"+newName+"'"); 
				try {
					if(nameCheck.next()){
						addOutGoingMessage(client,"message","Name already exists!");
					}else{
						Server.userDB.updateDB("update user_character set Name = '"+newName+"' where lower(Name) = '"+charName.toLowerCase()+"' limit 1");
						...
						for (Map.Entry<Integer, Client> entry : Server.clients.entrySet()) {
							...
							if(s.Ready){
								if(s.playerCharacter.getName().toLowerCase().equals(charName.toLowerCase())){
									addOutGoingMessage(s,"backtologin","Your name is inappropriate and has been changed by admins");
	...
	private static boolean levelFourControls(Client client, String chatText){
		...
		if(chatText.toLowerCase().startsWith("/ability ")){
			...
			try{
				...
				if(ServerGameInfo.abilityDef.containsKey(abilityId)){
					...
					addOutGoingMessage(client,"message",ServerGameInfo.abilityDef.get(abilityId).getName()+" added!");
				}
		...
		if(chatText.toLowerCase().startsWith("/bring ")){
			...
			if(player != null){
				teleportAdmin(player, "/gotoxyz "+client.playerCharacter.getX()+","+client.playerCharacter.getY()+","+client.playerCharacter.getZ());
			}
			return true;
		}
		...
		if(chatText.toLowerCase().startsWith("/setclass ")){
			...
			if(classInfo.length == 3){
				try{
					...
					// Save class change
					ResultSet classCheck = Server.userDB.askDB("select Id from character_class where ClassId = "+classId+" and CharacterId = "+client.playerCharacter.getDBId());
					...
					Server.userDB.updateDB("update character_class set Type = 0 where CharacterId = "+client.playerCharacter.getDBId()+" and Type = "+classType);
					
					if(hasClass){
						Server.userDB.updateDB("update character_class set Type = "+classType+", ClassLevel = "+level+" where CharacterId = "+client.playerCharacter.getDBId()+" and ClassId = "+classId);
					}else{
						Server.userDB.updateDB("insert into character_class (CharacterId, ClassId, ClassLevel, ClassXP, Type) values ("+client.playerCharacter.getDBId()+","+classId+","+level+",0,"+classType+")");
					}
				...
				addOutGoingMessage(client, "message", "/setclass classType (1 = primary, 2 = secondary),classId,classLevel");
				...
				try{
					...
					if(player != null){
						if(XPTables.nextLevelXP.containsKey(level)){
							...
							addOutGoingMessage(client, "message", "Changed player "+player.playerCharacter.getName()+" level to "+level);
						}
					}
		...
		if(chatText.toLowerCase().startsWith("/spawn ")){
			...
			try{
				...
				if(spawnId > 0){
					...
					if(spawnId < 87 && spawnId != 83){
						m = new Npc(ServerGameInfo.creatureDef.get(spawnId), client.playerCharacter.getX(), client.playerCharacter.getY(), client.playerCharacter.getZ());
						m.setAggroType(2);
					}
				}
			}catch(NumberFormatException e){
			...
			if(success && m != null){
				Server.WORLD_MAP.addMonsterSpawn(m, client.playerCharacter.getX(), client.playerCharacter.getY(), client.playerCharacter.getZ());
			}else{
				addOutGoingMessage(client, "message", "/spawn monsterId,gloomyNr,titanOrNot -> ex: /spawn 4,0,0");
			}
			return true;
		}
				...
				try{
					...
					if(skillId > 0 && level > 0 && level < 200){
						if(client.playerCharacter.getSkill(skillId) != null){
							Server.userDB.updateDB("update character_skill set Level = "+level+" where SkillId = "+skillId+" and CharacterId = "+client.playerCharacter.getDBId());
							client.playerCharacter.getSkill(skillId).setLevel(level);
							addOutGoingMessage(client,"message",client.playerCharacter.getSkill(skillId).getName()+" level set to "+level);
						}
					}
				}catch(NumberFormatException e){
					// NOT A NUMBER!
			...
			if(creatureInfo.length == 3){
				...
				try {
					...
					if(c.getName() != null){
						...
						if(foundStat){
							Server.gameDB.updateDB("update creature set "+statName+" = "+statValue+" where Id = "+creatureId);
	...
	private static boolean levelFiveControls(Client client, String chatText){
			...
			if(playerInfo.length == 2){
				try{
					...
					if(player != null){
						...
						addOutGoingMessage(client,"message","You have given "+player.playerCharacter.getName()+" "+xp+" XP");
					}else{
			...
			if(playerInfo.length == 2){
				try{
					...
					if(player != null){
						...
						Server.userDB.updateDB("update user_character set Bounty = "+bounty+" where Id = "+player.playerCharacter.getDBId());
						addOutGoingMessage(client,"message","You have given "+player.playerCharacter.getName()+" "+bounty+" in bounty");
					}else{
						addOutGoingMessage(client,"message","Player is not online!");
					}
				}catch(NumberFormatException e){
		...
		if(chatText.toLowerCase().startsWith("/giveskill ")){
			...
			if(skillInfo.length == 3){
				try{
					...
					if(player != null){
						...
						if(skill != null){
							...
							player.playerCharacter.saveInfo();
							addOutGoingMessage(player,"message","Your skill "+skill.getName()+" has been changes, please re-log to view changes.");
						}
					}else{
						addOutGoingMessage(client,"message","Player is not online!");
					}
				...
				addOutGoingMessage(client,"message","1 = Blades, 2 = Axes, 3 = Bows, 4 = Blunts, 5 = Unarmed, 6 = Fire, 7 = Ice, 8 = Shock, 9 = Cooking, 11 = Fishing, 12 = Herbalism");
			}
			return true;
		}
				...
				try{
					...
					if(adminLevel < 5){
						...
						if(player != null){
							...
							Server.userDB.updateDB("update user_character set AdminLevel = "+adminLevel+" where Id = "+player.playerCharacter.getDBId());
							addOutGoingMessage(player,"message","You've been granted admin rights!");
						
						}else{
							// Player not online, check db for character
							try {
								ResultSet charInfo = Server.userDB.askDB("select Id from user_character where lower(Name) = '"+charName.toLowerCase()+"'");
								if(charInfo.next()){
									foundPlayer = true;
									Server.userDB.updateDB("update user_character set AdminLevel = "+adminLevel+" where Id = "+charInfo.getInt("Id"));
								}
								charInfo.close();
							} catch (SQLException e) {
						...
						if(foundPlayer){
							addOutGoingMessage(client,"message","You've given "+charName+" admin level "+adminLevel+"!");
						}else{
			...
			if(skinInfo.length > 1){
				try{
					...
					if(ServerGameInfo.itemDef.containsKey(skinId)){
						// Check if user exist
						try {
							ResultSet playerInfo = Server.userDB.askDB("select Id, Name from user_character where Id = "+charId);
							if(playerInfo.next()){
								String playerName = playerInfo.getString("Name");
								Server.userDB.updateDB("insert into character_skin (CharacterId, ItemId) values ("+charId+","+skinId+")");
								addOutGoingMessage(client, "message", "Skin "+ServerGameInfo.itemDef.get(skinId).getName()+" given to "+playerName);
							}
							playerInfo.close();
						} catch (SQLException e) {
							// TODO Auto-generated catch block
		...
		if(chatText.toLowerCase().startsWith("/give ")){
			...
			if(giftInfo.length == 3){
				...
				ResultSet chestItems = Server.userDB.askDB("select InventoryPos from character_item where CharacterId = "+charId);
				try {
				...
				if(foundSpace){
					Server.userDB.updateDB("insert into character_item (CharacterId, ItemId, Equipped, InventoryPos, Nr, ModifierId, MagicId, GraphicsNr) values ("+charId+","+itemId+",0,'"+freeX+","+freeY+"',1,"+modifierId+",0,0)");
					addOutGoingMessage(client,"message","Item "+itemId+", modifier "+modifierId+" added to personal chest of char "+charId);
				}
			}else{
				addOutGoingMessage(client,"message","Wrong command, written /give charid,itemid,modifierid");
			}
			return true;
		}
	...
	private static void movePlayer(Client client, String playerName){
		...
		ResultSet playerInfo = Server.userDB.askDB("select Id, CheckpointId from user_character where lower(Name) like '"+playerName+"'");
		try {
			if(playerInfo.next()){
				int playerId = playerInfo.getInt("Id");
				int CheckpointId = playerInfo.getInt("CheckpointId");

				ResultSet checkpointInfo = Server.mapDB.askDB("select X,Y,Z from checkpoint where Id = "+CheckpointId);
				if(checkpointInfo.next()){
					...
					Server.userDB.updateDB("update user_character set X = "+checkpointX+", Y = "+checkpointY+", Z = "+checkpointZ+" where Id = "+playerId);
	...
	private static void teleportAdmin(Client client, String message){
		...
		if(message.startsWith("/gotoxyz ")){
			...
			ResultSet targetInfo = Server.userDB.askDB("select X, Y, Z from user_character where lower(Name) like '"+targetName+"'");
			try {
				if(targetInfo.next()){
					...
					for(int i = -1; i < 2; i++){
						for(int j = -1; j < 2; j++){
							if(Server.WORLD_MAP.getTile(targetX+i, targetY+j,targetZ) != null){
								if(Server.WORLD_MAP.getTile(targetX+i, targetY+j,targetZ).isPassableType() && Server.WORLD_MAP.getTile(targetX+i, targetY+j,targetZ).getDoorId() == 0){
									targetX += i;
									targetY += j;
									foundSpot = true;
									break;
		...
		if(teleportOk){
			...
			for(Iterator<Npc> iter = Server.WORLD_MAP.getMonsters().values().iterator();iter.hasNext();){  
				Npc m = iter.next();
				if(m.isAggro() && m.getAggroTarget().getCreatureType() == CreatureType.Player && m.getAggroTarget().getDBId() == client.playerCharacter.getDBId()){
					m.turnAggroOff();
				}
			}
			...
			for (Map.Entry<Integer, Client> entry : Server.clients.entrySet()) {
				...
				if(s.Ready){
					if(s.playerCharacter.getDBId() != client.playerCharacter.getDBId() && isVisibleForPlayer(s.playerCharacter,client.playerCharacter.getX(),client.playerCharacter.getY(),client.playerCharacter.getZ())){
			...
			Server.WORLD_MAP.getTile(targetX,targetY,targetZ).setOccupant(CreatureType.Player,client.playerCharacter);
	
			Server.userDB.updateDB("update user_character set X = "+targetX+", Y = "+targetY+", Z = "+targetZ+" where Id ="+client.playerCharacter.getDBId());
			...
			for (Map.Entry<Integer, Client> entry : Server.clients.entrySet()) {
				...
				if(s.Ready){
					if(s.playerCharacter.getDBId() != client.playerCharacter.getDBId() && isVisibleForPlayer(s.playerCharacter,client.playerCharacter.getX(),client.playerCharacter.getY(),client.playerCharacter.getZ())){
						...
						if(client.playerCharacter.getShip() != null && client.playerCharacter.getShip().isShow()){
							addOutGoingMessage(s,"goboat",client.playerCharacter.getSmallData()+";"+client.playerCharacter.getShip()+",1");
						}
					}
				}
			}
	...
	private static void banPlayer(Client client, String playerName, int hours){
		ResultSet playerInfo = Server.userDB.askDB("select Id, UserId from user_character where lower(Name) like '"+playerName+"'");
		try {
			if(playerInfo.next()){
				...
				try {
					...
					if(hours < 100){
						addOutGoingMessage(client,"message","You have banned "+playerName+" for "+hours+" hours");
					}else{
			...
			if(playerInfo.next()){
				...
				try {
					...
					if(!ip.equals("not found")){
						WebHandler.sendPost("http://www.bluesaga.org/server/banIp.php","ip="+ip+"&check="+check+"&admin="+client.playerCharacter.getName());
	...
	public static void giveQuest(Client client, String charName, int questId){
		ResultSet playerInfo = Server.userDB.askDB("select Id from user_character where lower(Name) = '"+charName+"'");
		try {
			if(playerInfo.next()){
				int playerId = playerInfo.getInt("Id");
				// Check if player has quest
				ResultSet questCheck = Server.userDB.askDB("select Id from character_quest where QuestId = "+questId+" and CharacterId = "+playerId);
				if(questCheck.next()){
					// Player has quest, complete it
					Server.userDB.updateDB("update character_quest set Status = 3 where QuestId = "+questId+" and CharacterId = "+playerId);
				}else{
					// Player does not have quest, add it as completed
					Server.userDB.updateDB("insert into character_quest (CharacterId, QuestId, Status) values ("+playerId+","+questId+",3)");
				}
				questCheck.close();
