...
public class SkinHandler extends Handler {
  ...
  public static void init() {
    DataHandlers.register("set_skin", m -> handleSetSkin(m));
    DataHandlers.register("change_character_skin", m -> handleChangeSkin(m));
    DataHandlers.register("reset_character_skin", m -> handleResetSkin(m));
  }
  ...
  public static String getClosetContent(Client client) {
    ...
    ResultSet closetInfo =
        Server.userDB.askDB(
            "select ItemId from character_skin where SkinType = 'Item' and CharacterId = "
                + client.playerCharacter.getDBId());

    int sizeW = 5;
    int sizeH = 7;

    content.append("Closet,").append(sizeW).append(',').append(sizeH).append('/');
    ...
    try {
      while (closetInfo.next()) {
        ...
        content
            ...
            .append(',')
            .append(skinItem.getName())
            .append(',')
            .append(skinItem.getSubType())
            .append(';');
      }
      closetInfo.close();
  ...
  public static String getCharacterSkins(Client client) {
    ...
    ResultSet closetInfo =
        Server.userDB.askDB(
            "select ItemId from character_skin where CharacterId = "
                + client.playerCharacter.getDBId()
                + " and SkinType = 'Character'");
  ...
  public static void handleSetSkin(Message m) {
  ...
  public static void handleChangeSkin(Message m) {
    Client client = m.client;
    int skinId = Integer.parseInt(m.message);
  ...
  public static void handleResetSkin(Message m) {
    Client client = m.client;
    client.playerCharacter.setCreatureId(client.playerCharacter.getBaseCreatureId());

    Server.userDB.updateDB(
        "update user_character set CreatureId = BaseCreatureId where Id = "
            + client.playerCharacter.getDBId());
    ...
    for (Map.Entry<Integer, Client> entry : Server.clients.entrySet()) {
      ...
      if (s.Ready
        ...
        addOutGoingMessage(
            ...
            client.playerCharacter.getSmallData()
                + ";"
                + client.playerCharacter.getBaseCreatureId());
      }
    }
  }
}
