...
public class CardHandler extends Handler {
  ...
  public static void setup() {
    ...
    ResultSet monsterInfo =
        Server.gameDB.askDB(
            "SELECT Id, Name, Level FROM creature WHERE Level <= "
                + maxLevel
                + " AND LootCopper > 0 ORDER BY Level ASC");
    int cardId = 315;
    try {
      while (monsterInfo.next()) {
        Server.gameDB.updateDB(
            "INSERT INTO item (Id, Name, Type, ReqLevel, DamageType, StatusEffects, Stackable, Value) VALUES ("
                + cardId
                + ",'"
                + monsterInfo.getString("Name")
                + " Card','Collector Card',0,'None','None',1, 0)");
        cardId++;
      }
      monsterInfo.close();
  ...
  public static void init() {
    ...
    ResultSet cards_id =
        Server.gameDB.askDB("SELECT Id, ItemId, GraphicsId, Type FROM card ORDER BY Id ASC");
        ...
        if (client.playerCharacter.getCardBook().contains(playerCard)) {
          ...
          if (playerCard.id == cardSlot) {
            ...
            Server.userDB.updateDB(
                "INSERT INTO character_card (CardId, CharacterId, DateTime) VALUES ("
                    + playerCard.id
                    + ","
            ...
            //if(!Server.DEV_MODE){
            String event =
                "?check=tjolahopp612&playerId="
                    + client.playerCharacter.getDBId()
                    + "&playerName="
                    + client.playerCharacter.getName()
                    + "&eventType=card_add&eventTargetName="
                    + playerCard.id
                    + "&eventTargetType=card&eventTargetId="
                    + playerCard.id;
  ...
  public static void removeCard(Client client, int cardId) {
    Server.userDB.updateDB(
        "DELETE FROM character_card WHERE CardId = "
            + cardId
            + " AND CharacterId = "
            + client.playerCharacter.getDBId());
    for (Card card : client.playerCharacter.getCardBook()) {
    ...
    if (client.playerCharacter.getCardBook().size() > 0) {
      ...
      String event =
          "?check=tjolahopp612&playerId="
              ...
              + "&playerName="
              + client.playerCharacter.getName()
              + "&eventType=card_lost&eventTargetName="
              + droppedCard.id
              + "&eventTargetType=card&eventTargetId="
              + droppedCard.id;
      WebHandler.callUrl("http://www.bluesaga.org/server/addEvent.php" + event);
  ...
  public static void checkEndGame(Client cardPicker) {
    ...
    if (winGame) {
      ...
      //if(!Server.DEV_MODE){
      String event =
          "?check=tjolahopp612&winnerId="
              + cardPicker.playerCharacter.getDBId()
              + "&nrCards="
              + cards.size();
