...
public class HitHandler extends Handler {
   ...
   * @return
   */
  public static void creatureGetHit(
      Creature TARGET,
      Creature ATTACKER,
      int damage,
      String damageType,
      String criticalOrMiss,
      boolean arena,
      ...
      if (TARGET.getCreatureType() == CreatureType.Monster) {
        ...
        if (ATTACKER != null) {
          ...
          if (ATTACKER.getCreatureType() == CreatureType.Monster
              && ATTACKER.getDBId() == targetMonster.getDBId()) {
            // Can't set itself as target
          } else {
            if (damage == targetMonster.getMostHitDamage()) {
              if (targetMonster.getOriginalAggroType() != 5
                  && targetMonster.getOriginalAggroType() != 3) {
      ...
      */
      ...
      if (targetDies) {
        ...
        for (Map.Entry<Integer, Client> entry : Server.clients.entrySet()) {
          ...
          if (other.Ready
              && Handler.isVisibleForPlayer(
                  other.playerCharacter, TARGET.getX(), TARGET.getY(), TARGET.getZ())) {
            // Send health status
            addOutGoingMessage(
                other,
                "changehealthstatus",
                TARGET.getSmallData() + ';' + TARGET.getHealthStatus());

            if (removeManaShield) {
              addOutGoingMessage(other, "statuseffect_remove", TARGET.getSmallData() + ";25");
            }
            if (TARGET.getCreatureType() == CreatureType.Player
                && other.playerCharacter.getDBId() == TARGET.getDBId()) {
        ...
        if (ATTACKER != null && !arena) {
          if (ATTACKER.getCreatureType() == CreatureType.Player) {
            ...
            if (targetDies) {
              ...
              if (TARGET.getCreatureType() == CreatureType.Player) {
                if (playerAttacker.getAdminLevel() < 3
                    && ATTACKER.getDBId() != TARGET.getDBId()
                    && pvpOk) {
                  // PVP
  ...
  public static void sendHitToClients(
      Creature TARGET,
      Creature ATTACKER,
      int damage,
      String damageType,
      ...
      if (s.Ready) {
        if (isVisibleForPlayer(s.playerCharacter, TARGET.getX(), TARGET.getY(), TARGET.getZ())) {
          addOutGoingMessage(
              s,
              "creature_hit",
              TARGET.getSmallData() + ';' + damage + ',' + damageType + ',' + criticalOrMiss);
        }
        ...
        if (targetDies) {
          if (isVisibleForPlayer(s.playerCharacter, TARGET.getX(), TARGET.getY(), TARGET.getZ())) {
            ...
            if (s.playerCharacter.getAggroTarget() != null) {
              if (s.playerCharacter
                      .getAggroTarget()
                      .getCreatureType()
                      .equals(TARGET.getCreatureType())
                  && s.playerCharacter.getAggroTarget().getDBId() == TARGET.getDBId()) {
      ...
      for (Map.Entry<Integer, Client> entry : Server.clients.entrySet()) {
        ...
        if (other.Ready
          ...
          if (TARGET.getCreatureType() == CreatureType.Player
              && other.playerCharacter.getDBId() == TARGET.getDBId()) {
            addOutGoingMessage(
                other, "update_bonusstats", other.playerCharacter.getBonusStatsAsString());
          }
        }
      }
    }
  ...
  public static void targetDeath(Creature TARGET, Creature ATTACKER, boolean arena) {
    ...
    for (int i = TARGET.getX() - 3; i < TARGET.getX() + 3; i++) {
      for (int j = TARGET.getY() - 3; j < TARGET.getY() + 3; j++) {
        if (Server.WORLD_MAP.getTile(i, j, TARGET.getZ()) != null) {
          if (Server.WORLD_MAP
                  .getTile(i, j, TARGET.getZ())
                  .getOccupantType()
                  .equals(TARGET.getCreatureType())
              && Server.WORLD_MAP.getTile(i, j, TARGET.getZ()).getOccupant().getDBId()
                  == TARGET.getDBId()) {
    ...
    for (Iterator<Npc> iter = Server.WORLD_MAP.getMonsters().values().iterator();
      ...
      Npc m = iter.next();
      if (m.isAggro()) {
        if (m.getAggroTarget().getCreatureType().equals(TARGET.getCreatureType())
            && m.getAggroTarget().getDBId() == TARGET.getDBId()) {
          m.turnAggroOff();
        }
      }
    }
