...
public class PvpHandler extends Handler {
	...
	public static boolean canAttackPlayer(PlayerCharacter attacker, PlayerCharacter target){
		...
		// No attacks indoors
		if(Server.WORLD_MAP.getTile(target.getX(), target.getY(), target.getZ()).getType().equals("indoors")){
			attackOk = false;
		}
		...
		if(ServerSettings.PVP){
			...
			if(attacker.getDBId() == target.getDBId()){
				attackOk = true;
			}else if(!Server.WORLD_MAP.getTile(target.getX(), target.getY(), target.getZ()).getType().equals("arena")){
				attackOk = false;
			}
		}
		return attackOk;
	...
	public static void setPlayerKillerMark(PlayerCharacter attacker, int pkMark){
		...
		Server.userDB.updateDB("update user_character set PlayerKiller = "+pkMark+" where Id = "+attacker.getDBId());

		String pkData = attacker.getSmallData()+";"+pkMark;

		for (Map.Entry<Integer, Client> entry : Server.clients.entrySet()) {
			Client s = entry.getValue();
			if (s.Ready && isVisibleForPlayer(s.playerCharacter, attacker.getX(), attacker.getY(), attacker.getZ())) {
