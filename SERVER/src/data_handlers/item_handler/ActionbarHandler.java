...
public class ActionbarHandler extends Handler {
  ...
  public static void handleAddActionBar(Message m) {
    ...
    if (actionType.equals("Ability")) {
      ...
      if (MouseItem != null) {
        if (MouseItem.getType().equals("Potion")
            || MouseItem.getType().equals("Scroll")
            || MouseItem.getType().equals("Eatable")) {
  ...
  public static boolean useItem(Client client, Item usedItem) {
    ...
    if (usedItem.getType().equals("Scroll")) {
      // Send use scroll
      addOutGoingMessage(
          client, "use_scroll", usedItem.getId() + "," + usedItem.getSubType() + ",Actionbar");
    } else {
