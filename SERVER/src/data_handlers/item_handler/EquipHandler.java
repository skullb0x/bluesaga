...
public class EquipHandler extends Handler {
  ...
  public static void init() {
    DataHandlers.register("equip", m -> handleEquip(m));
    DataHandlers.register("unequip", m -> handleUnequip(m));
    DataHandlers.register("statuswindow", m -> handleStatusWindow(m));
  }

  public static void handleEquip(Message m) {
      ...
      if (equipOk) {
        ...
        if (!equipOk) {
          ...
          addOutGoingMessage(
              client,
              "you_equip_no",
              "This item is two-hands! You can't carry an offhand and a two hands weapon at the same time.");
        }
      }
      ...
      if (equipOk) {
        ...
        if (oldEquip != null) {
          ...
          Server.userDB.updateDB(
              "update character_item set Equipped = 0, InventoryPos = 'Mouse' where Id = "
                  + oldEquip.getUserItemId());
        }
        ...
        Server.userDB.updateDB(
            "update character_item set Equipped = 1, InventoryPos = 'None' where CharacterId = "
                + client.playerCharacter.getDBId()
                + " and Id = "
                + equippedItem.getUserItemId());
        ...
        addOutGoingMessage(
            ...
            equippedItem.getId()
                ...
                + equippedItem.getUserItemId()
                + ";"
                + equippedItem.getType()
                + ";"
                + equippedItem.getClassId()
                + ";"
                + oldEquipId
                + ";"
                + oldEquipUserItemId
        ...
        addOutGoingMessage(
            client, "you_equip_no", "You don't have the requirements for this item!");
      }
    }
  }

  public static void handleUnequip(Message m) {
    Client client = m.client;
    String unequipType = m.message;
    ...
    if (unequippedItem != null) {
      ...
      // Add item to mouse
      addOutGoingMessage(
          client, "addmouseitem", unequippedItem.getId() + ";" + unequippedItem.getType());
  ...
  public static void handleStatusWindow(Message m) {
    if (!"info".equals(m.message)) return;
    Client client = m.client;
    // EQUIP INFO
    String equipInfo = "";
    ...
    equipInfo =
        headEquipId
            ...
            + weaponEquipId
            + ","
            + weaponEquipUserItemId
            + ","
            + offHandEquipId
            + ","
            + offHandEquipUserItemId
            + ","
            + amuletEquipId
            + ","
            + amuletEquipUserItemId
            + ","
            + artifactEquipId
            + ","
            + artifactEquipUserItemId;
    ...
    */
    ...
    if (weaponItem != null) {
      ...
      if (loseItem) {
        ...
        Server.userDB.updateDB(
            "delete from character_item where Id = " + weaponItem.getUserItemId());
    ...
    if (offhandItem != null) {
      ...
      if (loseItem) {
        ...
        Server.userDB.updateDB(
            "delete from character_item where Id = " + offhandItem.getUserItemId());
    ...
    if (amuletItem != null) {
      ...
      if (loseItem) {
        ...
        Server.userDB.updateDB(
            "delete from character_item where Id = " + amuletItem.getUserItemId());
    ...
    if (artifactItem != null) {
      ...
      if (loseItem) {
        ...
        Server.userDB.updateDB(
            "delete from character_item where Id = " + artifactItem.getUserItemId());
