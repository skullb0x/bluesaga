...
public class InventoryHandler extends Handler {
	...
	public static void handleData(Client client, String message){
		...
		if(message.startsWith("<addmouseitem>")){
			String itemInfo[] = message.substring(14).split(";");
			...
			if(containerType == 0){
				...
				ResultSet rs = Server.userDB.askDB("select Id, ItemId, Nr, ModifierId, MagicId from character_item where InventoryPos = '"+posX+","+posY+"' and CharacterId = "+client.playerCharacter.getDBId());

				try {
					if(rs.next()){

						// PLAYER HAS ITEM AT THAT POSITION
						Server.userDB.updateDB("update character_item set InventoryPos = 'Mouse' where Id = "+rs.getInt("Id"));
						...
						if(ServerGameInfo.itemDef.containsKey(rs.getInt("ItemId"))){
							...
							addOutGoingMessage(client,"addmouseitem",rs.getInt("ItemId")+";"+mouseItem.getType());
				...
				ResultSet rs = Server.userDB.askDB("select Id, ItemId, Nr, ModifierId, MagicId from user_chest where Pos = '"+posX+","+posY+"' and UserId = "+client.UserId);
				...
				try {
					if(rs.next()){
						...
						if(ServerGameInfo.itemDef.containsKey(rs.getInt("ItemId"))){
							...
							Server.userDB.updateDB("delete from user_chest where Id = "+mouseItem.getUserItemId());
		...
		if(message.startsWith("<splitmouse>")){
			boolean okToSplit = false;

			if(client.playerCharacter.getMouseItem() == null){
				String splitInfo[] = message.substring(12).split(";");
				...
				if(number > 0){
					...
					ResultSet itemCheck = Server.userDB.askDB("select Id, ItemId, Nr from character_item where InventoryPos = '"+invPos+"' and CharacterId = "+client.playerCharacter.getDBId());
					...
					try {
						if(itemCheck.next()){
							...
							if(itemCheck.getInt("Nr") > number){
								// SPLIT
								Server.userDB.updateDB("update character_item set Nr = Nr - "+number+" where Id = "+itemCheck.getInt("Id"));
								Server.userDB.updateDB("insert into character_item (CharacterId, ItemId, Equipped, InventoryPos, Nr) values ("+client.playerCharacter.getDBId()+","+itemCheck.getInt("itemId")+",0,'Mouse',"+number+")");
								okToSplit = true;
							}else if(itemCheck.getInt("Nr") == number){
								// MOVE ALL TO MOUSE
								Server.userDB.updateDB("update character_item set InventoryPos = 'Mouse' where Id = "+itemCheck.getInt("Id"));
								okToSplit = true;
							}
							if(okToSplit){
		...
		if(message.startsWith("<moveitem>")){
			String itemInfo[] = message.substring(10).split(";");
			...
			if(client.playerCharacter.getMouseItem() != null){
				...
				ResultSet mouseItemInfo = Server.userDB.askDB("select Nr, ModifierId, MagicId from character_item where CharacterId = "+client.playerCharacter.getDBId()+" and InventoryPos = 'Mouse'"); 
				try {
				...
				if(containerType == 0){
					...
					if(!addSpecialItem){
						...
						ResultSet checkInvPos = Server.userDB.askDB("select Id, ItemId, Nr from character_item where CharacterId = "+client.playerCharacter.getDBId()+" and InventoryPos = '"+posX+","+posY+"'");
						try {
							if(checkInvPos.next()){
								// ITEM ALREADY THERE!
								...
								if(oldInvItem.getId() == itemToMove.getId() && oldInvItem.getStacked() < oldInvItem.getStackable()){
									...
									Server.userDB.updateDB("update character_item set Nr = Nr + "+nrItemsToAdd+" where ItemId = "+oldInvItemId+" and InventoryPos = '"+posX+","+posY+"' and CharacterId = "+client.playerCharacter.getDBId());
									
									// IF ITEMS LEFT AFTER FILLING UP STACK
									int nrItemsLeft = nrItems - nrItemsToAdd;
									
									if(nrItemsLeft > 0){
										Server.userDB.updateDB("update character_item set Nr = "+nrItemsLeft+" where ItemId = "+oldInvItemId+" and InventoryPos = 'Mouse' and CharacterId = "+client.playerCharacter.getDBId());
										...
										Server.userDB.updateDB("delete from character_item where ItemId = "+oldInvItemId+" and InventoryPos = 'Mouse' and CharacterId = "+client.playerCharacter.getDBId());
									...
									Server.userDB.updateDB("update character_item set InventoryPos = '"+posX+","+posY+"' where InventoryPos = 'Mouse' and CharacterId = "+client.playerCharacter.getDBId());

									// PUT EXISTING ITEM TO MOUSE
									addOutGoingMessage(client, "addmouseitem", oldInvItemId+";"+oldInvItem.getType());

									Server.userDB.updateDB("update character_item set InventoryPos = 'Mouse' where Id = "+oldInvItemUserItemId);
						...
						addOutGoingMessage(client, "moveitem", itemToMove.getId()+";"+itemToMove.getUserItemId()+";"+posX+";"+posY+";"+nrItems+";"+containerType);

					}
				}else if(containerType == 1){
					// PERSONAL CHEST
					// CHECK IF ITEM ALREADY EXIST AT LOCATION
					ResultSet checkInvPos = Server.userDB.askDB("select Id, ItemId, Nr, ModifierId, MagicId from user_chest where UserId = "+client.UserId+" and Pos = '"+posX+","+posY+"'");
					try {
						if(checkInvPos.next()){
							// ITEM ALREADY THERE!
							oldInvItemUserItemId = checkInvPos.getInt("Id");
							...
							if(oldInvItem.getId() == itemToMove.getId() && oldInvItem.getStacked() < oldInvItem.getStackable()){
								...
								// IF SAME TYPE OF ITEM THEN STACK THEM
								Server.userDB.updateDB("update user_chest set Nr = Nr + "+nrItemsToAdd+" where ItemId = "+oldInvItemId+" and Pos = '"+posX+","+posY+"' and UserId = "+client.UserId);
								...
								Server.userDB.updateDB("update user_chest set ItemId = "+itemToMove.getId()+", Nr = "+nrItems+", ModifierId = "+itemToMove.getModifierId()+", MagicId = "+itemToMove.getMagicId()+" where Id = "+oldInvItemUserItemId);
								...
								Server.userDB.updateDB("insert into character_item (CharacterId, ItemId, Nr, Equipped, InventoryPos, ModifierId, MagicId) values ("+client.playerCharacter.getDBId()+","+oldInvItem.getId()+","+oldInvItem.getStacked()+",0,'Mouse',"+oldInvItem.getModifierId()+","+oldInvItem.getMagicId()+")");
								addOutGoingMessage(client, "addmouseitem", oldInvItemId+";"+oldInvItem.getType());
								client.playerCharacter.setMouseItem(oldInvItem);
							...
							// FREE SPACE
							// MOVE NEW ITEM TO POSITION
							Server.userDB.updateDB("insert into user_chest (UserId, ItemId, Nr, Pos, ModifierId, MagicId) values ("+client.UserId+","+itemToMove.getId()+","+nrItems+",'"+posX+","+posY+"',"+itemToMove.getModifierId()+","+itemToMove.getMagicId()+")");
	...
	public static boolean addSpecialItemToInventory(Client client, Item itemToMove){
		...
		if(itemToMove.getType().equals("Key")){
			...
			ResultSet keyInfo = Server.userDB.askDB("select KeyId from character_key where KeyId = "+itemToMove.getId()+" and CharacterId = "+client.playerCharacter.getDBId());
			...
			if(addSpecialItem){
				Server.userDB.updateDB("insert into character_key (KeyId, CharacterId) values ("+itemToMove.getId()+","+client.playerCharacter.getDBId()+")");
				addOutGoingMessage(client,"message",itemToMove.getName()+" #messages.inventory.added_to_keychain");

				Server.userDB.updateDB("delete from character_item where CharacterId = "+client.playerCharacter.getDBId()+" and InventoryPos = 'Mouse'");
				client.playerCharacter.setMouseItem(null);
			...
			ResultSet recipeInfo = Server.userDB.askDB("select Id from character_recipe where RecipeId = "+itemToMove.getId()+" and CharacterId = "+client.playerCharacter.getDBId());
			...
			if(addSpecialItem){
				Server.userDB.updateDB("insert into character_recipe (RecipeId, CharacterId) values ("+itemToMove.getId()+","+client.playerCharacter.getDBId()+")");
				addOutGoingMessage(client,"message",itemToMove.getName()+" #messages.inventory.added_to_craftbook");
	...
	public static void removeCopperFromInventory(Client client, int copperToRemove){
		...
		moneyUpdate = Server.userDB.askDB("select Id, Nr from character_item where ItemId = 36 and CharacterId = "+client.playerCharacter.getDBId());

		try {
			while(moneyUpdate.next() && copperToRemove > 0){
				int copperLeft = copperToRemove - moneyUpdate.getInt("Nr");
				if(copperLeft >= 0){
					Server.userDB.updateDB("delete from character_item where Id = "+moneyUpdate.getInt("Id"));
					copperToRemove = copperLeft;
				}else{
					int restCopper = moneyUpdate.getInt("Nr") - copperToRemove;
					copperToRemove = 0;
					Server.userDB.updateDB("update character_item set Nr = "+restCopper+" where Id = "+moneyUpdate.getInt("Id"));
				}
			}
			moneyUpdate.close();
		} catch (SQLException e) {
		...
		// REMOVE SILVER
		if(copperToRemove > 0){
			moneyUpdate = Server.userDB.askDB("select Id, Nr from character_item where ItemId = 35 and CharacterId = "+client.playerCharacter.getDBId());
			...
			try {
				while(moneyUpdate.next() && copperToRemove > 0){
					...
					if(copperLeft >= 0){
						...
						copperToRemove = 0;
						if(silverLeft > 0){
							Server.userDB.updateDB("update character_item set Nr = "+silverLeft+" where Id = "+moneyUpdate.getInt("Id"));
						}else{
							Server.userDB.updateDB("delete from character_item where Id = "+moneyUpdate.getInt("Id"));
						}
						if(copperRest > 0){
		...
		// REMOVE GOLD
		if(copperToRemove > 0){
			moneyUpdate = Server.userDB.askDB("select Id, Nr from character_item where ItemId = 34 and CharacterId = "+client.playerCharacter.getDBId());
			...
			try {
				while(moneyUpdate.next() && copperToRemove > 0){
					...
					if(copperLeft >= 0){
						...
						copperToRemove = 0;
						if(goldLeft > 0){
							Server.userDB.updateDB("update character_item set Nr = "+goldLeft+" where Id = "+moneyUpdate.getInt("Id"));
						}else{
							Server.userDB.updateDB("delete from character_item where Id = "+moneyUpdate.getInt("Id"));
						}
						if(copperRest > 0){
		...
		if(!addSpecialItem){
			...
			if(!newItemPos.equals("None")){
				if(newItem.getType().equals("Money")){
					...
					addOutGoingMessage(client,"message",newItem.getName()+" #messages.inventory.added_to_inventory");
				}
				...
				if(stacked.equals("newpos")){
					Server.userDB.updateDB("insert into character_item (CharacterId, ItemId, Equipped, Nr, InventoryPos, ModifierId, MagicId) values("+client.playerCharacter.getDBId()+","+newItem.getId()+",0,"+totalItems+",'"+InventoryPos+"',"+newItem.getModifierId()+","+newItem.getMagicId()+")");
				}else{
					Server.userDB.updateDB("update character_item set Nr = "+totalItems+" where ItemId = "+newItem.getId()+" and CharacterId = "+client.playerCharacter.getDBId()+" and InventoryPos = '"+InventoryPos+"'");
				}
	...
	public static boolean useItem(Client client, Item usedItem, int posX, int posY){
		...
		ResultSet rs = Server.userDB.askDB("select Id, ItemId, Nr, InventoryPos from character_item where ItemId = "+usedItem.getId()+" and InventoryPos = '"+posX+","+posY+"' and CharacterId = "+client.playerCharacter.getDBId());
			...
			if(rs.next()){
				...
				if(usedItem != null){
					...
					if(usedItem.getType().equals("Scroll")){
						// SCROLL
						addOutGoingMessage(client, "use_scroll", usedItem.getUserItemId()+","+usedItem.getSubType()+",Inventory");
					}else if(usedItem.getType().equals("Readable")){
						// READABLE ITEM
						addOutGoingMessage(client, "readable", usedItem.getName()+";"+usedItem.getDescription());
					}else{
						// USEABLE ITEM
	...
	public static int countPlayerItem(Client client, int itemId){
		...
		ResultSet itemInfo = Server.userDB.askDB("select ItemId, Nr from character_item where CharacterId = "+client.playerCharacter.getDBId()+" and ItemId = "+itemId+" and InventoryPos <> 'None' and InventoryPos <> 'Mouse' and InventoryPos <> 'Actionbar'");
		try {
			while(itemInfo.next()){
	...
	public static boolean removeNumberOfItems(Client client, int itemId, int nrItems){
		...
		ResultSet itemInfo = Server.userDB.askDB("select Id, Nr from character_item where CharacterId = "+client.playerCharacter.getDBId()+" and ItemId = "+itemId+" and InventoryPos <> 'Mouse' and Equipped = 0 and InventoryPos <> 'Actionbar'");
		try {
			while(itemInfo.next() && nrItems > 0){
				...
				if(nrLeftInInv > 0){
					removedNr = nrItems;
					Server.userDB.updateDB("update character_item set Nr = "+nrLeftInInv+" where Id = "+itemInfo.getInt("Id"));
				}else{
				...
				if(newItem.getStackable() > 1){
					for(int i = 0; i < inventorySize; i++){
						for(int j = 0; j < inventorySize; j++){
							...
							if(invItem != null){
								if(invItem.getId() == newItem.getId() && invItem.getStacked() + newItem.getStacked() <= invItem.getStackable()){
	...
	public void removeItemFromInventory(Client client, int posX, int posY, int nr){
		ResultSet removeInfo = Server.userDB.askDB("select Nr from character_item where InventoryPos = '"+posX+","+posY+"' and CharacterId = "+client.playerCharacter.getDBId());

		try {
			if(removeInfo.next()){
				if(removeInfo.getInt("Nr") > 1){
					Server.userDB.updateDB("update character_item set Nr = Nr - 1 where InventoryPos = '"+posX+","+posY+"' and CharacterId = "+client.playerCharacter.getDBId());
				}else{
					Server.userDB.updateDB("delete from character_item where InventoryPos = '"+posX+","+posY+"' and CharacterId = "+client.playerCharacter.getDBId());
				}
