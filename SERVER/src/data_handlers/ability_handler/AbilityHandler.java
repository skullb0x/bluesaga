...
public class AbilityHandler extends Handler {
	...
	public static void playerUseAbility(Client client, Ability ABILITY, int goalX, int goalY, int goalZ){
				...
				if(equipType.equals("Weapon")){
					if(client.playerCharacter.getEquipment(equipType) != null){
						...
						if(classId > 0){
							...
							if(classId == 101 && (client.playerCharacter.getEquipment(equipType).getId() == 173 
			...
			if(ABILITY.getManaCost() <= client.playerCharacter.getMana()){
				...
				if(ABILITY.isReady()){
					...
					if(Server.WORLD_MAP.getTile(goalX, goalY, goalZ) != null){
						...
						attackOk = BattleHandler.checkAttackOk(client.playerCharacter.getX(),client.playerCharacter.getY(),client.playerCharacter.getZ(),goalX, goalY, goalZ);
						...
						if(ABILITY.getAbilityId() == 84 || ABILITY.getAbilityId() == 83){
							if(!Server.WORLD_MAP.getTile(goalX, goalY, goalZ).isPassableForPlayer(client.playerCharacter)){
								attackOk = false;
							}
						}
							...
							if(haveTarget){
								...
								if(distToTarget <= abilityRange || ABILITY.isTargetSelf()){
									...
									if(ABILITY.getAbilityId() == 84){
										StatusEffectHandler.addStatusEffect(client.playerCharacter,new StatusEffect(42));
									}
									...
									if(ABILITY.getAbilityId() == 27){
										...
										MonsterHandler.alertNearMonsters(client.playerCharacter,goalX,goalY,goalZ,true);

									}else if(ABILITY.getAbilityId() == 31 && client.playerCharacter.getPkMarker() == 0){
										// SOUL STONE
										...
										for (Entry<Integer, Client> entry : Server.clients.entrySet()) {
											...
											if(other.Ready && other.playerCharacter.getDBId() != client.playerCharacter.getDBId() && isVisibleForPlayer(other.playerCharacter,client.playerCharacter.getX(),client.playerCharacter.getY(),client.playerCharacter.getZ())){
												addOutGoingMessage(other,"creature_remove",client.playerCharacter.getSmallData());
											}
										}
										...
										addAbilityEvent(ABILITY, client.playerCharacter, goalX,goalY,goalZ,castingSpeed);
	...
	public static void addAbilityEvent(Ability ABILITY, Creature CASTER, int goalX, int goalY, int goalZ, int delay){
		AbilityEvents.add(new AbilityEvent(ABILITY, CASTER, goalX, goalY, goalZ, delay));
	}

	public static void addProjectile(Ability ABILITY, int startX, int startY, int startZ, int goalX, int goalY, int goalZ){
		...
		if(ABILITY.getAbilityId() == 74){
			...
			Projectile newProjectile2 = new Projectile(ABILITY, startX, startY, startZ, goal2X, goal2Y, goalZ);
			Projectile newProjectile3 = new Projectile(ABILITY, startX, startY, startZ, goal3X, goal3Y, goalZ);
		...
		if(ABILITY != null){
			...
			if(CASTER != null){
				if(Server.WORLD_MAP.getTile(CASTER.getX(), CASTER.getY(), CASTER.getZ()) != null){
					if(Server.WORLD_MAP.getTile(CASTER.getX(), CASTER.getY(), CASTER.getZ()).getType().equals("arena")){
						arena = true;
					}
				}
			}
					...
					if(CASTER != null){
						if(CASTER.getAggroTarget() != null){
							...
							while(step < 10 && thrustOk && !thrustDone){
								...
								checkTile = Server.WORLD_MAP.getTile(CASTER.getX()+moveX, CASTER.getY()+moveY, CASTER.getZ());
							...
							if(!AoE.equals("")){
								...
								if(CASTER.getCreatureType() == CreatureType.Player){
									...
									Tile oldPosTile = Server.WORLD_MAP.getTile(CASTER.getX(), CASTER.getY(), CASTER.getZ());
									...
									Tile newPosTile = Server.WORLD_MAP.getTile(CASTER.getX(), CASTER.getY(), CASTER.getZ());
						...
						if(TILE != null && TILE.isPassableType()){
							...
							if(!ABILITY.getSpawnIds().equals("None")){
								...
								for(String spawnId: spawnIds){
									...
									if(spawnTile != null){
										Npc m = new Npc(ServerGameInfo.creatureDef.get(sId),spawnTile.x,spawnTile.y,tileZ);
										m.setAggroType(2);
								...
								if(TARGET != null){
									...
									if(CASTER != null && TARGET.getCreatureType().equals(CASTER.getCreatureType()) && TARGET.getDBId() == CASTER.getDBId() && !ABILITY.isBuffOrNot()){
											// Do not hit caster with its own spell if not a buff
									}else{
										HitHandler.creatureGetHit(TARGET, CASTER, damage, ABILITY.getDamageType(),"false",arena, ABILITY.getStatusEffects());
									}
								}	
							}
			...
			if(!tileData.equals("None")){
				tileData += "/";
				String seInfo = "";
				for(Iterator<StatusEffect> iter = ABILITY.getStatusEffects().iterator();iter.hasNext();){  
			...
			if(s.Ready){
				if(s.playerCharacter != null){
					if(!s.playerCharacter.isDead()){
						...
						// MANA FROM POTION
						int regainedMana = s.playerCharacter.regainPotionMana(s.playerCharacter.potionRegainManaTick);
						if(regainedMana != 0){
							addOutGoingMessage(s,"stat","Mana;"+s.playerCharacter.getMana());
						}

						// HEALTH FROM POTION
						int regainedHealth = s.playerCharacter.regainPotionHealth(s.playerCharacter.potionRegainHealthTick);
						if(regainedHealth != 0){
						...
						if(s.playerCharacter.regainItrUpdate() ){
							...
							Tile playerTile = Server.WORLD_MAP.getTile(s.playerCharacter.getX(), s.playerCharacter.getY(), s.playerCharacter.getZ());
							if(playerTile != null){
	...
	public static int calculateAbilityDamage(Ability ABILITY, Creature TARGET){
		...
		if(ABILITY.getDamage() > 0){
			if(CASTER != null){
				if(CASTER.getCreatureType() == CreatureType.Monster){
					...
					aDmg = ((CASTER.getStat("INTELLIGENCE") + 10.0f)/10.0f) * ((modifLevel+ 12.0f) / 12.0f) * ABILITY.getDamage() / 10.0f;
				}
		...
		if(CASTER != null){
			if(TARGET.getCreatureType() == CreatureType.Player && CASTER.getCreatureType() == CreatureType.Player){
