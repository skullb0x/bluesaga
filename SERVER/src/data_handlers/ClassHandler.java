...
public class ClassHandler extends Handler {
			...
			if(playerClass != null){
				...
				if(xp > 0){
					if(playerClass.addXP(xp)){
						...
						addOutGoingMessage(client,"class_levelup",playerClass.id+","+playerClass.level+","+playerClass.nextXP);
					}else {
						addOutGoingMessage(client,"class_xp",playerClass.id+","+playerClass.getXp());
					}	
				}
				
				Server.userDB.updateDB("update character_class set ClassXP = "+playerClass.getXp()+", ClassLevel = "+playerClass.level+" where CharacterId = "+client.playerCharacter.getDBId()+" and ClassId = "+playerClass.id);
			}
		}
	}
		...
		if(playerClass != null){
			...
			if(playerSkill != null){
				if(playerSkill.addSP(1)){
					...
					addOutGoingMessage(client,"sp_levelup",playerSkill.getId()+","+playerSkill.getLevel()+","+playerSkill.getSP()+","+XPTables.nextLevelSP.get(playerSkill.getLevel()+1));
				}else{
					addOutGoingMessage(client,"set_sp",playerSkill.getId()+","+playerSkill.getSP());
				}
				Server.userDB.updateDB("update character_skill set SP = "+playerSkill.getSP()+", Level = "+playerSkill.getLevel()+" where CharacterId = "+client.playerCharacter.getDBId()+" and SkillId = "+classId);
			}
		}
	}
	...
	public static boolean learnClass(Client client, int classId, int classType){
		...
		if(classType == 1){
			...
			if(client.playerCharacter.getBaseClass().id != ServerGameInfo.classDef.get(classId).baseClassId){
				classChangeOk = false;
				addOutGoingMessage(client, "message", "#messages.classes.cant_learn#"+client.playerCharacter.getBaseClass().name+".\n#messages.classes.learn_secondary");
			}
		...
		if(classChangeOk){
			...
			if(hasClass){
				Server.userDB.updateDB("update character_class set Type = "+classType+" where CharacterId = "+client.playerCharacter.getDBId()+" and ClassId = "+classId);
			}else{
				Server.userDB.updateDB("insert into character_class (CharacterId, ClassId, ClassLevel, ClassXP, Type) values ("+client.playerCharacter.getDBId()+","+classId+",1,0,"+classType+")");
			}
