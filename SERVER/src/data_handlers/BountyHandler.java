...
public class BountyHandler extends Handler {
  ...
  public static void handleMostWanted(Message m) {
    ...
    ResultSet rs =
        Server.userDB.askDB(
            "select Name, Bounty from user_character where Bounty > 0 order by Bounty desc limit 10");

    StringBuilder wantedData = new StringBuilder(1000);

    try {
      while (rs.next()) {
        wantedData.append(rs.getString("Name")).append(',').append(rs.getInt("Bounty")).append(';');
      }
      rs.close();
  ...
  public static void changeBounty(PlayerCharacter victim, PlayerCharacter killer) {
    ...
    Server.userDB.updateDB(
        "update user_character set Bounty = " + victimBounty + " where Id = " + victim.getDBId());

    killer.setBounty(killerBounty);
    Server.userDB.updateDB(
        "update user_character set Bounty = " + killerBounty + " where Id = " + killer.getDBId());

    if (!ServerSettings.DEV_MODE) {
      WebHandler.callUrl(
          "http://www.bluesaga.org/server/updateBounty.php?id="
              + victim.getDBId()
              + "&bounty="
              + victimBounty);
      WebHandler.callUrl(
