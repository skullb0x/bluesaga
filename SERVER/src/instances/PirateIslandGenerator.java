...
public class PirateIslandGenerator {
							...
							for(Point p: l){
								...
								if(spawnChance == 0 ){
									...
									if(worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ) != null){
										...
										if(spawnSquid && worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ).isWater()){
											// SPAWN SQUID
											spawnOk = true;
										}else if(spawnMonster && !worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ).isWater()){
											// OTHER MONSTERS
											if(worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ).isPassable()){
												spawnOk = true;
											}
										}
										
										if(spawnOk){
											Npc tempNpc = new Npc(ServerGameInfo.creatureDef.get(monsterId),worldMapX,worldMapY,worldMapZ);
			...
			if(tileNr == 4 || tileNr == 6){
				...
				if(chanceOfTrees == 0 && (keyX+keyY) % 2 == 0){
					...
					while(nrTrees > 0){
						...
						if(mapTiles.get(treeX+","+treeY+","+treeZ) != null && (mapTiles.get(treeX+","+treeY+","+treeZ) == 4 || mapTiles.get(treeX+","+treeY+","+treeZ) == 6)){
							String treeId = "tree1";
	...
	private static void fixTextures(HashMap<String,Tile> worldMapTiles){
		...
		while (it.hasNext()) {
			...
			if(tileNr == 2){
				...
				if(!saveEntrancePos){
					...
					Server.mapDB.updateDB("UPDATE door SET GotoX = "+worldMapX+", GotoY = "+worldMapY+", GotoZ = "+worldMapZ+" WHERE Id = "+entranceDoorId);
			...
			if(tileNr < 6){
				if(equalTile(mapTiles,(keyX-1)+","+(keyY)+","+keyZ,tileNr) && equalTile(mapTiles,(keyX)+","+(keyY-1)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX+1)+","+(keyY)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX)+","+(keyY+1)+","+keyZ,tileNr)){
					tileName = lowerType+"DR"; 
				}else if(equalTile(mapTiles,(keyX+1)+","+(keyY)+","+keyZ,tileNr) && equalTile(mapTiles,(keyX)+","+(keyY-1)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX-1)+","+(keyY)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX)+","+(keyY+1)+","+keyZ,tileNr)){
					tileName = lowerType+"DL"; 
				}else if(equalTile(mapTiles,(keyX-1)+","+(keyY)+","+keyZ,tileNr) && equalTile(mapTiles,(keyX)+","+(keyY+1)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX+1)+","+(keyY)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX)+","+(keyY-1)+","+keyZ,tileNr)){
					tileName = lowerType+"UR"; 
				}else if(equalTile(mapTiles,(keyX+1)+","+(keyY)+","+keyZ,tileNr) && equalTile(mapTiles,(keyX)+","+(keyY+1)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX-1)+","+(keyY)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX)+","+(keyY-1)+","+keyZ,tileNr)){
					tileName = lowerType+"UL"; 
				}else if(equalTile(mapTiles,(keyX-1)+","+(keyY)+","+keyZ,tileNr) && equalTile(mapTiles,(keyX)+","+(keyY+1)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX-1)+","+(keyY+1)+","+keyZ,tileNr)){
				...
				}else if(equalTile(mapTiles,(keyX-1)+","+(keyY)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX+1)+","+(keyY)+","+keyZ,tileNr)){
					tileName = lowerType+"R"; 
				}else if(equalTile(mapTiles,(keyX+1)+","+(keyY)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX-1)+","+(keyY)+","+keyZ,tileNr)){
					tileName = lowerType+"L"; 
				}else if(equalTile(mapTiles,(keyX)+","+(keyY-1)+","+keyZ,tileNr) && !equalTile(mapTiles,(keyX)+","+(keyY+1)+","+keyZ,tileNr)){
		...
		while (it.hasNext()) {
			...
			if(tileNr == 6){
				...
				worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ).setType("grass", ""+randomNr, 1);
			}
		}
	}
	...
	public static int placeCaveEntrances(HashMap<String,Tile> worldMapTiles, int nrCaves){
		...
		for(Point m: ArchipelagoGenerator.mountains){
			...
			for(Point p: l){
				if(p.getX() != 0 || p.getY() != 0){
					...
					if(worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ) != null){
						if(worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ).getName().equals("grassD")){
							worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ).setType("grasscliff", "grassEntranced", 1);
		...
		for(String caveCoord: caveEntrances){
			...
			dg.pirateIslandGenerate(worldMapTiles, monsterGroup,tileX, tileY, tileZ, pirateIslandZ-(monsterGroup*5));
	...
	public static void placeHoles(HashMap<String,Tile> worldMapTiles, int nrHoles){
		...
		while (it.hasNext()) {
			...
			if(tileType == 4 && distanceToCave <= 0){
				...
				nrHoles--;
				worldMapTiles.get(worldMapX+","+worldMapY+","+worldMapZ).setType("grass", "hole", 1);
