...
public class Creature {
	...
	private int AggroType; // 0 = not moving and no aggro, 1 = moving and no aggro, 2 = moving and aggro
	...
	public void draw(Graphics g, int x, int y){
		if(!hidden){
			...
			*/
			if(!Dead){
				Shadow.draw(x-(SizeWidth-1)*25,y+5,SizeWidth*50,50, new Color(255,255,255,255));
			}
			...
			if(FaceDir.equals("LEFT")){
				if(Dead){
					animation.getImage(0).drawFlash(x, y+dY-5 - Math.round(AnimationDeathY) - 50*(SizeHeight-1), 50*SizeWidth, 50*SizeHeight,  new Color(255,255,255,100));
				}else{
					animation.draw(x, y+dY-5 - 50*(SizeHeight-1), 50*SizeWidth, 50*SizeHeight,animationColor); 
				...
				if(Dead){
					animation.getImage(0).drawFlash(x+50*SizeWidth, y+dY-5 - Math.round(AnimationDeathY) - 50*(SizeHeight-1), -50*SizeWidth, 50*SizeHeight, new Color(255,255,255,100));
				}else{
					animation.draw(x+50*SizeWidth, y+dY-5 - 50*(SizeHeight-1), -50*SizeWidth, 50*SizeHeight,animationColor); 
	...
	public void draw(Graphics g, int x, int y, float[][][] lightValue){
		if(!hidden){
			...
			if(!Dead){
				...
				image.drawFlash(x, y+dY-5 - Math.round(AnimationDeathY) - 50*(SizeWidth-1), 50*SizeHeight, 50*SizeWidth,  new Color(255,255,255,100));	
			}
	...
	public void hitByAbility(Ability ability, int damage){
		...
		hitColor1 = new Color(abilityColor.getRed(),abilityColor.getGreen(),abilityColor.getBlue(),100);
	...
	public void attackAnimation() {
		...
		if(AnimationAttackItr == 20){
			...
			timerAttack.schedule( new TimerTask(){
    	        public void run() {
    	           attackAnimation();
    	        }
    	      }, 10);
