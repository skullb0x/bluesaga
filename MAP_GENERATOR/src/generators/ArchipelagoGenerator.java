...
public class ArchipelagoGenerator {
  ...
  public static void generate(int nrIslands) {
    ...
    System.out.println("nr of islands: " + nrIslands);
    int islandX = 0;
    int islandY = 0;
    ...
    for (int n = 0; n < nrIslands; n++) {
      ...
      int chanceOfIsland = RandomUtils.getInt(0, 3);
      ...
      int distanceToNextMountain = RandomUtils.getInt(200, 500);
      ...
      for (int s = 0; s < islandSize; s++) {
        ...
        if (chanceOfIsland == 0) {
          ...
          // Island
          Iterator<Entry<Point, Integer>> it =
              GeneratorBrush.getIslandBrush().entrySet().iterator();
          ...
          while (it.hasNext()) {
            ...
            if (!mapTiles.containsKey(
                (islandX + b.getKey().x) + "," + (islandY + b.getKey().y) + "," + islandZ)) {
              mapTiles.put(
                  (islandX + b.getKey().x) + "," + (islandY + b.getKey().y) + "," + islandZ,
                  b.getValue());
            } else {
